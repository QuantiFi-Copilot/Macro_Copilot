...
**Backend artifact:** `calculate_ois_forward_rate_tool` (generic_runnable)
...
Implied forward rate between two points on an OIS curve via a dual-compounding bootstrap (simple interest for T ≤ 1Y, annual compounding for T > 1Y).  Two input modes — tenor-based or date-based — both returning forward rate, daily change, rolling z-score, trailing range, and time series.
