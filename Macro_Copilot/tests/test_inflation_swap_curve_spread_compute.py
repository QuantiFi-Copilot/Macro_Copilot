"""
test_inflation_swap_curve_spread_compute.py — Unit tests for the
ZCIS curve-spread primitive.

Mirrors ``test_breakeven_curve_spread_compute.py`` (the closest
sibling) — both primitives compose a level / spot read twice and
inherit the inner guards transitively.  This module differs in
that it composes the ZCIS rate-level primitive (PERCENT-units
endpoint reads, four-conjunct SELECT guard) rather than the linker
breakeven primitive.
...
Covers:
  ...
  6. Composition guard inheritance: missing endpoint legs surface
     ...
     propagated with short-tenor / long-tenor attribution.
...
``tests/test_inflation_swap_curve_spread_sql_validation.py``).
"""
...
def _patched_fetch_factory(legs: dict):
    ...
    def _stub(*, engine, curve_family, tenor, field_name, start_date):
        ...
        if key in legs:
            return legs[key]
        # Unknown combinations surface as "no rows" so a typo fails
        # fast through the inner level primitive's controlled error
        # envelope.
...
def _build_legs_for_usd_5s10s(
    *,
    ...
    """Default canonical two-leg fixture for USD_ZCIS 5s10s.

    The two legs drift by different amounts so the resulting
    curve spread is NOT constant — necessary for the z-score /
...
def _run(params, legs, *, config=None):
    with patch(
        ...
        return calculate_inflation_swap_curve_spread(
...
class TestBundledConfig:
    ...
    def test_methodology_planned_extensions_populated(self):
        cfg = load_tool_config(CONFIG_PATH)
        # Catalog requires at minimum: forward, cross-market spread,
        # swap-breakeven basis, plus the trailing-window
        # configurability extension.
    ...
    def test_methodology_what_it_does_carries_disclosure(self):
        ...
        assert "long_zcis_pct" in text and "short_zcis_pct" in text
...
class TestComputeHappyPath:
    def test_default_config_returns_well_formed_output(self):
        legs = _build_legs_for_usd_5s10s()
        ...
        assert cm["spread_label"] == "USD_ZCIS 5s10s"

    def test_spread_formula_is_correct(self):
        """Hard-pin the spread formula on a synthetic fixture with
        constant endpoint ZCIS rates.  With short=2.0%, long=3.0%,
        expected spread = (3.0 - 2.0) * 100 = 100bps.
        """
        days = 800
        legs = {
            ("USD_ZCIS", "5Y"): _synthetic_pillar_df(
        ...
        assert "error" not in out, out.get("error")
        cm = out["current_metrics"]
        assert cm["short_zcis_rate_pct"] == pytest.approx(2.0, abs=1e-6)
        assert cm["long_zcis_rate_pct"] == pytest.approx(3.0, abs=1e-6)
        assert cm["spread_bps"] == pytest.approx(100.0, abs=0.05)
    ...
    def test_non_integer_year_fractions_handled(self):
        """Boundary case: 1Y/5Y curve.  short_years=1.0,
        long_years=5.0.  Verify the validator + display handles
        the smaller pillars in the universe."""
        days = 800
        legs = {
        ...
        assert "error" not in out, out.get("error")
        cm = out["current_metrics"]
        assert cm["short_years"] == pytest.approx(1.0, abs=1e-4)
        assert cm["long_years"] == 5.0
        # spread = (2.5 - 2.0) * 100 = 50 bps
        assert cm["spread_bps"] == pytest.approx(50.0, abs=0.05)
...
class TestCompositionGuards:
    """Curve-spread composes the level primitive twice; the
    level's no-proxy guard (instrument_type + pricing_type +
    curve_family + tenor SELECT filter) must fire on each
    endpoint and surface as a controlled error envelope when
    violated.
    """

    def test_missing_short_endpoint_returns_error_envelope(self):
        legs = _build_legs_for_usd_5s10s()
        # Drop the short-tenor leg entirely.
    ...
    def test_unknown_curve_family_returns_error_envelope(self):
        ...
        it with short-tenor attribution (the short leg is tried
        first)."""
        legs: dict = {}
        params = InflationSwapCurveSpreadInput(
            curve_family="UST",
...
class TestPerTradeDateAlignment:
    ...
    clean step.  The spread layer never adds its own ffill across
    the join.
    """
    ...
    def test_missing_endpoint_date_is_dropped(self):
        ...
        so the spread layer's strict inner-join MUST drop them
        ...
        # outside the gap.  Synthetic data is 800 business days;
        # the displayed window is roughly the last ~260.  Index
        # ``len - 130`` lands near the middle of that window.
        all_dates = sorted(
        ...
        # level — the spread inner-join MUST drop those dates.  At
        # least 2 dates fall past the limit (gap_size - ffill_limit
        # = 2), so a non-empty subset is checked.
...
class TestMethodologyLabelThreading:
    def test_methodology_label_sourced_from_yaml(self):
        ...
        custom_text = (
            "CUSTOM-ZCIS-CURVE-SPREAD-DISCLOSURE-99 same-curve ZCIS "
            "tenor spread long_zcis_pct - short_zcis_pct"
        )
        out = _run(
            params, legs,
...
class TestCanonicalTimeSeries:
    def test_canonical_spread_uses_bps_units_and_naming(self):
        ...
        assert ts["units"] == "bps"
        assert ts["series_name"] == "usd_zcis_5y_10y_zcis_curve_spread"
    ...
    def test_canonical_zscore_uses_z_score_units_and_naming(self):
        ...
        assert (
            ts["series_name"]
            == "usd_zcis_5y_10y_zcis_curve_spread_zscore"
        )
