...
2. **`event_category` is a free `VARCHAR(32)`, not a `CHECK`-constrained enum.** The valid set — `economic_release`, `central_bank_meeting`, `auction` — is documented here and as a Python constant (`EVENT_CATEGORIES` in `database/database.py`). This is consistent with how `instrument_master.asset_class` and `instrument_type` are already modelled (free `VARCHAR`, closed-family discipline enforced at the contract/code level per P8, not via a DB `CHECK`). A DB `CHECK` would be a new pattern in `macro_data` and would turn every category addition into a DDL constraint swap.
...
- **`_normalize_event_record(record)`** — pure-Python; validates `event_type`, `event_category`, `country`, `release_date` are present, warns (does not abort) if `event_category` is outside `EVENT_CATEGORIES`, and returns a uniform-key dict (every `event_calendar` value column present, `None` default) so SQLAlchemy bulk insert generates one coherent column list. Mirrors `_normalize_otr_record`.
...
- `event_category` has no DB-level `CHECK`. Mitigated: the valid set is a documented Python constant and is enforced at the contract level, consistent with `asset_class` / `instrument_type`.
