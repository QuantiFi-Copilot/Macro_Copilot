...
- **`instrument_master` has typed columns (`curve_family`, `tenor`, `country`, `currency`, `underlying_index`, `contract_code`, `cusip`, `isin`, …) shaped for rates today**. Asset-class-specific typed columns will be added via schema migrations (P8-flavoured decisions); domain-specific fields land in the `attributes` JSONB until they are query-hot enough to promote.
...
- **`instrument_type`** — a coarse category the substrate uses for filtering. Asset-class-specific values; examples: `sovereign_benchmark`, `sovereign_cash_bond`, `ois_swap`, `bond_future`, `inflation_indexed_bond`, `policy_future` for rates today. For FX it would be `spot`, `forward`, `vol_surface`, etc.
