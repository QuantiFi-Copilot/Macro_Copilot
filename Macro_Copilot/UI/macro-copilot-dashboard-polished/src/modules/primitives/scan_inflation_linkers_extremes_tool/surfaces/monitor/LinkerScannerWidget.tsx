// ============================================================================
// LinkerScannerWidget — Monitor bento tile for the universe-wide linker
// REAL-YIELD extremes scanner.
// ----------------------------------------------------------------------------
// Parameterised on (scope, top_n, min_abs_z_score) where ``scope`` is the
// linker curve_family or the magic "ALL" value for the full universe.
// Fetches the SAME typed-detail endpoint the Build views use —
// /api/v1/rates/detail/linkers-scanner — per the standalone-bridge
// contract (methodology_exposure.md §5.4).
//
// Per rendering_density.md §8 the Monitor surface is INHERENTLY COMPACT.
// The tile surfaces the desk-canonical "where is the linker curve
// stretched today?" read — a flagged-count chip, the top-3 ranked rows,
// and the load-bearing INDEX-FAMILY + MARKET-STRUCTURE caveat.
// ============================================================================
...
import type { ScanInflationLinkersExtremesOutput } from '@/types/rates';
import {
  WidgetHeader,
  WidgetBody,
  WidgetProvenance,
...
export function LinkerScannerWidget({ params }: Props) {
  const fetchParams = useMemo<LinkersScannerDetailParams>(() => {
  ...
  const { data, error, isLoading } = useFetchDetailLinkersScanner(fetchParams);
  ...
  return (
    <>
      <WidgetHeader
        kicker={`LINKERS · ${scopeLabel}`}
        title="Linker Extremes"
        meta={
          <span
            className={cn(
      ...
      <WidgetBody className="flex flex-col gap-2 px-5 pb-3">
        ...
        {/* Per rendering_density.md §2.2 + §12 methodology MUST be reachable
            ...
            MARKET-STRUCTURE caveat in a single line.  The full multi-line
...
function useFetchDetailLinkersScanner(params: LinkersScannerDetailParams) {
