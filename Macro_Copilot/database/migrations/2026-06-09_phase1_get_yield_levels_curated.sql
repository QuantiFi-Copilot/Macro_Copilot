-- ============================================================================
-- Migration STUB: macro_data.tool_metadata for `get_yield_levels_tool`
...
SET
    theoretical_reference = $tref$STUB - see rates_agent/sovereign_bonds/tools/get_yield_levels/config.yaml methodology block.$tref$,
    ...
    updated_at = NOW()
WHERE tool_name = 'get_yield_levels_tool';
...
SELECT
    'get_yield_levels_tool', 'sovereign_bonds', 'snapshot', '{}'::jsonb,
    $tref$STUB$tref$,
    $klim$STUB$klim$,
    $narr$STUB$narr$
WHERE NOT EXISTS (SELECT 1 FROM macro_data.tool_metadata WHERE tool_name = 'get_yield_levels_tool');
