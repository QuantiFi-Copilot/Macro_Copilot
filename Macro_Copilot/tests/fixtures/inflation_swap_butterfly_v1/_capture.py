...
only shape difference is THREE endpoint tenors per scenario instead
of two.
...
from rates_agent.inflation_swaps.tools.inflation_swap_butterfly.compute import (  # noqa: E402
    CONFIG_PATH as _ISB_CONFIG_PATH,
)
...
_CASES: list[dict] = [
    {
        "fixture_name": "usd_zcis_5s10s30s_365d",
        "params": {
    ...
    {
        "fixture_name": "eur_zcis_2s5s10s_365d",
        "params": {
    ...
    {
        "fixture_name": "gbp_zcis_5s10s30s_365d",
        "params": {
...
def _capture_one(case: dict, engine, captured_at: str, db_name: str) -> dict:
    params = InflationSwapButterflyInput(**case["params"])
    frozen_today = date.today()

    cfg = load_tool_config(_ISB_CONFIG_PATH)
    ...
    expected_output = calculate_inflation_swap_butterfly(
    ...
    return {
        ...
        "tool_function": "calculate_inflation_swap_butterfly",
        "capture": {
