-- ============================================================================
-- Migration STUB: macro_data.tool_metadata for `calculate_cross_country_real_yield_spread_simple_tool`
...
SET
    theoretical_reference = $tref$STUB - see rates_agent/inflation_indexed_bonds/tools/calculate_cross_country_real_yield_spread_simple/config.yaml methodology block.$tref$,
    ...
    updated_at = NOW()
WHERE tool_name = 'calculate_cross_country_real_yield_spread_simple_tool';
...
SELECT
    'calculate_cross_country_real_yield_spread_simple_tool', 'inflation_indexed_bonds', 'cross_market_rv', '{}'::jsonb,
    $tref$STUB$tref$,
    $klim$STUB$klim$,
    $narr$STUB$narr$
WHERE NOT EXISTS (SELECT 1 FROM macro_data.tool_metadata WHERE tool_name = 'calculate_cross_country_real_yield_spread_simple_tool');
