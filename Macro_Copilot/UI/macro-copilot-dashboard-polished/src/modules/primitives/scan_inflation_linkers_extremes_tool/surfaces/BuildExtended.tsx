...
// linker REAL-YIELD extremes.  Mirrors the scan_inflation_swaps_extremes
// sibling shape (identity row → top-right summary cards → controls strip →
// distribution histogram → ranked detail table → methodology card →
// lineage footer) and reuses the shared ControlsStrip / MethodologyCard /
// LineageFooter / FreshnessPill elements so the chrome matches the rest
// of the Build catalogue.
//
...
import type { ScanInflationLinkersExtremesResultRow } from '@/types/rates';
...
const CURVE_FAMILY_OPTIONS: ReadonlyArray<{ value: string; label: string }> = [
  { value: ALL_CURVE_FAMILIES_VALUE, label: 'All Linkers (TIPS / Gilts / OATi / RRB)' },
  ...LINKER_SCANNER_CURVE_OPTIONS,
];
...
const BuildExtended: React.FC<BuildExtendedProps> = ({
  ...
  const highCount = rows.filter((r) => (r.z_score_real_yield ?? 0) > 0).length;
  const lowCount = rows.filter((r) => (r.z_score_real_yield ?? 0) < 0).length;
  const extremeCount = rows.filter(
    (r) => Math.abs(r.z_score_real_yield ?? 0) >= 2.0,
  ).length;
  ...
  return (
    <div
      ...
      data-testid="linker-scanner-extended"
    >
      {/* ---------- Title section ---------- */}
      <section className="grid grid-cols-1 gap-4 border-b border-line-subtle px-6 pt-5 pb-5 lg:grid-cols-[1fr_auto]">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[11px] uppercase tracking-wide text-fg-muted">
            ...
            <span className="font-semibold">LINKER UNIVERSE SCAN</span>
      ...
      <div className="grid grid-cols-1 gap-3 px-6 pt-4 lg:grid-cols-3">
        <SummaryCallout
          ...
          count={highCount}
          hint="Stems where the 252d-rolling real yield sits above its trailing-year mean by ≥ threshold."
        />
        <SummaryCallout
          tone="extreme-down"
          label="Biggest moves (low)"
          count={lowCount}
          hint="Stems where the 252d-rolling real yield sits below its trailing-year mean by ≥ threshold."
        />
        <SummaryCallout
          tone="elevated"
      ...
      <div className="px-6 pt-4">
        <RankedTable
          ...
          onOpenInBuild={(row) => {
            // Open the per-bond real_yield_level primitive for the
            // selected (curve_family, tenor).  The companion primitive
...
function IndexFamilyCard({
  ...
  // CPI-U / RPI / HICP / CAN-CPI caveat is visible without scrolling.
  const families = new Set<string>();
  for (const r of rows) {
    const meta = linkerFamilyFor(r.curve_family);
...
function RankedTable({
  ...
  isLoading?: boolean;
  errorMessage?: string;
  onOpenInBuild: (row: ScanInflationLinkersExtremesResultRow) => void;
}) {
  return (
        ...
        <table className="w-full min-w-[820px] text-[12.5px]">
          <thead>
            <tr className="border-b border-line-subtle text-left text-[10.5px] uppercase tracking-wide text-fg-muted">
              ...
              <th className="px-3 py-2 text-right font-normal">Real Yield</th>
          ...
          <tbody>
            {errorMessage ? (
              <tr>
                <td colSpan={10} className="px-4 py-6 text-center text-coral-300">
                  {errorMessage}
                </td>
              </tr>
            ) : isLoading ? (
              ...
              <tr>
                <td colSpan={10} className="px-4 py-6 text-center text-fg-secondary">
              ...
              rows.map((row) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`px-3 py-2 text-right font-mono tabular-nums ${toneTextClass(toneForChangeBps(row.daily_change_bps))}`}>
                      {formatChangeBps(row.daily_change_bps)}
                    </td>
                    <td className={`px-3 py-2 text-right font-mono tabular-nums ${toneTextClass(toneForChangeBps(row.monthly_change_bps))}`}>
                      {formatChangeBps(row.monthly_change_bps)}
                    </td>
  ...
  return (
    <div className="card flex flex-col gap-3 px-4 py-4">
      ...
      <div className="flex items-start gap-2 rounded-md border border-line-subtle bg-surface-overlay px-3 py-2 text-[11px] leading-snug text-fg-secondary">
        ...
        <span>
          Click <span className="font-semibold text-fg-primary">Open in Build</span> on any row to drill into the per-bond
