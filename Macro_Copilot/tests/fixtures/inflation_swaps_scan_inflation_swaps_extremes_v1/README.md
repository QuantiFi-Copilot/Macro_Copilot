# `inflation_swaps_scan_inflation_swaps_extremes_v1` parity fixtures

Snapshots of the **real production output** of
`calculate_scan_inflation_swaps_extremes` captured against the live
TimescaleDB.  The companion test at
`tests/test_scan_inflation_swaps_extremes_parity.py` reloads each
...
baseline.  `scan_inflation_swaps_extremes` is the first universe-
scan primitive in the `inflation_swaps` domain (catalog id
`inflation_swaps__scan_inflation_swaps_extremes`, build_order 25)
— the morning ZCIS quoted-rate extremes sweep — so it ships with
parity coverage rather than being grandfathered debt.

The fixture locks in the *current production behaviour* — the exact
output the tool produces on real Bloomberg-shaped ZCIS universe
...
fetcher, a methodology disclosure rename, an additive change to the
extended `fetch_scan_universe_reference` helper) fails the parity
test loudly.

The captured baseline includes real-data quirks that synthetic
inputs won't reproduce: per-currency ZCIS holiday calendars, days
where one curve family reports independently, the underlying
universe's tenor-coverage symmetry (USD_ZCIS / EUR_ZCIS / GBP_ZCIS
each expose 1/2/3/5/10/20/30Y — 7 tenors × 3 curve families = 21
stems).
...
```json
{
  ...
  "tool_module": "rates_agent.inflation_swaps.tools.scan_inflation_swaps_extremes",
  "tool_function": "calculate_scan_inflation_swaps_extremes",
  "capture": {
    ...
    "raw_rows_sha256": "<hex>",
    "resolved_curve_families": ["USD_ZCIS", "EUR_ZCIS",
                                "GBP_ZCIS"]
  },
  "input": {
...
- **`input.raw_field_rows`** is the long-format DataFrame the
  ...
  `instrument_type='inflation_swap'` + `field_name='PX_MID'`.
  The parity test feeds this back into the tool through a mocked
  fetcher.
- **`input.raw_reference_rows`** is the per-(curve_family, tenor,
  contract_code) reference rows the (extended)
  `fetch_scan_universe_reference` helper returns
  (`maturity_date`, `country`, `vendor_ticker`,
  `underlying_index`).  The `underlying_index` column was added
  to the helper's projection in the same PR — additive, non-
  breaking for the linker scanner's call site.
...
with the inflation_swaps playbook data loaded (the
`PX_MID` field across the ZCIS universe).  Defaults match
the project's `docker-compose.yml`:

```bash
# 1. Make sure TimescaleDB is up and the ZCIS universe is
#    ingested with PX_MID.
...
(`full_universe_top5_zfloor`) which pins the full V1 ZCIS
universe scan with `top_n=5` and `min_abs_z_score=0.0` — small
enough that the fixture is reviewable but exercises the ranking,
threshold, reference-attach (including the new `underlying_index`
column), and disclosure-attach paths.  Add more cases to
`_CASES` in `_capture.py` if additional configurations should be
parity-pinned (e.g. a USD_ZCIS-only subset, a high-threshold
variant).
...
```bash
pytest tests/test_scan_inflation_swaps_extremes_parity.py -v
```
