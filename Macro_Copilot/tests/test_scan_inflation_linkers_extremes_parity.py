"""
test_scan_inflation_linkers_extremes_parity.py — Snapshot parity tests
                                                  for the linker
                                                  universe-wide
                                                  extremes scan
=====================================================================
...
``calculate_scan_inflation_linkers_extremes`` for one or more
representative scan invocations (default: the full V1 linker
...
``scan_inflation_linkers_extremes`` is a new primitive, so the
parity coverage ships with it rather than being grandfathered
debt.

The fixtures are captured against a live TimescaleDB by
``tests/fixtures/inflation_linkers_scan_inflation_linkers_extremes_v1/_capture.py``;
...
------------
For each fixture in
``tests/fixtures/inflation_linkers_scan_inflation_linkers_extremes_v1/``:
...
2. Reconstruct the two long-format DataFrames the tool's DB
   fetchers would return for the linker universe.
3. Patch
   ``rates_agent.inflation_indexed_bonds.tools.scan_inflation_linkers_extremes.compute.fetch_scan_universe``
   and
...
4. Build a ``ScanInflationLinkersExtremesInput`` from the recorded
   params and call ``calculate_scan_inflation_linkers_extremes(
   engine=None, params=...)``.
...
----------------------------------------
``calculate_scan_inflation_linkers_extremes`` already rounds every
numeric output (z-score to 4 dp, yield to 4 dp, bps changes to
2 dp), so on the same inputs the result *should* be bit-identical
...
``tests/fixtures/inflation_linkers_scan_inflation_linkers_extremes_v1/README.md``.
"""
...
FIXTURES_DIR = (
    ...
    / "inflation_linkers_scan_inflation_linkers_extremes_v1"
)
...
class _FrozenDateForScan(date):
    ...
    Patched in place of
    ``rates_agent.inflation_indexed_bonds.tools.scan_inflation_linkers_extremes.compute.date``
...
def test_scan_inflation_linkers_extremes_parity(fixture_path: Path) -> None:
    """Run the captured inputs through
    ``calculate_scan_inflation_linkers_extremes`` with the DB
    ...
    # 3. Build the input.
    params = ScanInflationLinkersExtremesInput(**fx["input"]["params"])
    ...
    def _scan_universe_side_effect(
        *, engine, instrument_type, field_name, start_date,
        ...
        # The tool reads default_field_name from config.yaml —
        # currently YLD_YTM_MID.  Route by field_name so a future
        # YAML rename gets the right series.
        if field_name == "YLD_YTM_MID":
        ...
        raise AssertionError(
            ...
            "YLD_YTM_MID. The YAML's default_field_name has "
    ...
    with patch(
        ...
        actual = calculate_scan_inflation_linkers_extremes(
...
def test_scan_inflation_linkers_extremes_parity_tamper_detected() -> None:
