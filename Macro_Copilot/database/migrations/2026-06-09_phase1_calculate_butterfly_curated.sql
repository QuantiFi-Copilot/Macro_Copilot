-- ============================================================================
-- Migration STUB: macro_data.tool_metadata for `calculate_butterfly_tool`
...
SET
    theoretical_reference = $tref$STUB - see rates_agent/sovereign_bonds/tools/calculate_butterfly/config.yaml methodology block.$tref$,
    ...
    updated_at = NOW()
WHERE tool_name = 'calculate_butterfly_tool';
...
SELECT
    'calculate_butterfly_tool', 'sovereign_bonds', 'curve_shape', '{}'::jsonb,
    $tref$STUB$tref$,
    $klim$STUB$klim$,
    $narr$STUB$narr$
WHERE NOT EXISTS (SELECT 1 FROM macro_data.tool_metadata WHERE tool_name = 'calculate_butterfly_tool');
