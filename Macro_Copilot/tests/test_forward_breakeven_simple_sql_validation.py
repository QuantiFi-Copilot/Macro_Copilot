#!/usr/bin/env python3
"""
test_forward_breakeven_simple_sql_validation.py — Linker forward-
breakeven validator.

Validate ``calculate_forward_breakeven_simple`` against an
independent SQL baseline run directly on
``macro_data.v_market_data_daily_enriched``.  The SQL baseline
reproduces the year-weighted-linear forward-breakeven math without
going through any of the Python tool's helpers, so a mismatch
surfaces a real divergence in methodology rather than a shared-code
coincidence.

Key forward-breakeven-specific differences from the spot
breakeven_inflation_simple validator:

  - SQL baseline pulls FOUR underlying yield series (nominal at
    start_tenor, linker at start_tenor, nominal at end_tenor,
    linker at end_tenor), aligns on trade_date, computes the two
    endpoint breakevens in BPS, then applies the year-weighted
    ...
    where T_short / T_long are the calendar year fractions of the
    two endpoint tenors (parsed via the same
    ``shared.analytics.curve_bootstrap.tenor_to_years`` helper the
    Python tool uses).

  - Adversarial pollution probes (linker→nominal, nominal→linker)
    AND adversarial cross-country probes (EUR-zone, cross-currency,
    plus a symmetric slot-swap) — same shape as the spot validator,
    composed via the forward primitive.
...
# (nominal_curve_family, linker_curve_family, start_tenor, end_tenor)
...
# Canonical regression cases — one per same-country pair, anchored to
# desk-recognised forward windows (5Y5Y, 5Y10Y, 2Y3Y).  All entries
# below are confirmed same-country / same-currency against
...
REGRESSION_CASES: List[Case] = [
    ("UST", "USD_TIPS", "5Y", "10Y"),       # 5Y5Y — canonical desk read
    ("UK_GILT", "GBP_LINKER", "5Y", "10Y"),
    ("FR_OAT", "EUR_FR_LINKER", "5Y", "10Y"),
    ("CANADA_GOVT", "CAD_RRB", "5Y", "10Y"),
]

# Tolerances follow the spot breakeven_inflation_simple validator's
# precision since the bps math + display precision are the same.
# Forward arithmetic divides by (T_long - T_short) which can amplify
# rounding error slightly on the bps surface; we widen the bps
# tolerance modestly to account for that without losing teeth.
TOLERANCE_BY_FIELD = {
    "forward_breakeven_pct": 0.00021,
    "forward_breakeven_bps": 0.021,
    ...
    "start_breakeven_bps": 0.011,
    "end_breakeven_bps": 0.011,
    "start_years": 1e-4,
    "end_years": 1e-4,
    "forward_breakeven_bps_row": 0.021,
    "z_score_row": 0.006,
}
...
# Adversarial cross-country rejection probes — same shape as the spot
# validator.
...
def choose_test_cases(
    ...
    """Pick (nominal, linker, start_tenor, end_tenor) cases from live
    metadata.

    Pulls (curve_family, tenor) coverage with sufficient observation
    count for both legs (nominal AND linker) at BOTH endpoint tenors,
    AND filters to *same-country / same-currency* pairs only.

    Constraint rationale: a forward bond-implied breakeven is by
    construction a same-country object (the nominal sovereign and
    linker legs must share country and currency).  The spot
    primitive's compute layer refuses cross-country pairs at runtime;
    this pool mirrors that contract so the validator only ever
    exercises pairs the tool will accept.
    """
    nominal_query = text(
        """
        SELECT
    ...
    # Group by (curve_family, country, currency) → set of tenors.
    ...
    for (n_cf, n_country, n_currency), n_tenors in nominal_tenors.items():
        for (l_cf, l_country, l_currency), l_tenors in linker_tenors.items():
            ...
            # Need at least two distinct tenors to form a forward.
            if len(common_tenors) < 2:
                continue
            # Build all valid (start, end) pairs where start < end by
            # year fraction.  Skip unparseable tenors silently.
...
def sql_baseline(
    ...
    *,
    ...
    """Independent SQL reproduction of the year-weighted forward-
    breakeven math.
    ...
    Algorithm (mirrors the Python tool's composition shape EXACTLY):
      1. Pull two endpoint breakeven series, EACH built per-tenor
         ...
         emits.  Critically the per-tenor ffill operates on a
         per-tenor date grid (only the dates relevant to that
         tenor's two legs), NOT a multi-tenor union — otherwise
         the SQL ffill would bridge gaps across more rows than the
         Python spot's per-tenor pivot does, producing extra
         pre-history that diverges from the Python tool.
      2. Inner-join the two endpoint breakeven series on
         trade_date — same as the forward primitive's outer
         alignment step.
      3. Apply the year-weighted linear forward formula and round
         to 2 decimals (matches the forward primitive's
    ...
    # computation in calculate_forward_breakeven_simple).  The
    # buffer is max(z_window, trailing_window) * buffer_multiplier
    # = 252 * 1.5 = 378.  Without trimming each per-tenor SQL BE
    # to this same window, the SQL retains pre-history that the
    # Python spot's display trim threw away → SQL's rolling z-score
    # source is longer than Python's, producing a real divergence
    # the validator should NOT mask.
    ...
    baseline_sql = text(
        ...
        WITH raw AS (
            ...
            WHERE field_name = :field_name
              AND tenor IN (:start_tenor, :end_tenor)
        ...
        -- Per-tenor pivot+ffill+dropna for the SHORT leg, mirroring
        -- the spot primitive's pivot_and_align_tenors call at
        -- tenor=:start_tenor.  The date grid here covers ONLY the
        -- dates relevant to the (nominal_short, linker_short) pair.
        short_raw AS (
        ...
        short_be_unfiltered AS (
            -- Spot breakeven at start_tenor in BPS, rounded to 2
            -- decimals — mirrors compute_spread_bps in the spot
            -- primitive.
            SELECT
                trade_date,
        ...
        short_be AS (
            -- Trim to extended_lookback days from this leg's own
            -- latest aligned trade_date — matches the spot
            -- primitive's display trim
            -- (display = wide.loc[wide.index >= as_of - lookback_days]
            --  with lookback_days = extended_lookback in this
            --  composition).  Without this trim the SQL retains
            -- pre-history that Python's spot threw away, producing
            -- a longer rolling-stat source than Python.
        ...
        -- Per-tenor pivot+ffill+dropna for the LONG leg, mirroring
        -- the spot primitive's pivot_and_align_tenors call at
        -- tenor=:end_tenor.
        long_raw AS (
            SELECT trade_date, curve_family, field_value
        ...
        long_be AS (
            -- Same per-tenor display trim as short_be — see comment
            -- on short_be CTE.
        ...
        -- Inner-join the two endpoint breakeven series — mirrors
        -- the forward primitive's outer pd.merge(how='inner').
        aligned AS (
            SELECT
                s.trade_date,
                s.be_short_bps,
        ...
        forward_rows AS (
            SELECT
                ...
                ROUND(
                    (
                        (
                            be_long_bps  * :t_long_years
                          - be_short_bps * :t_short_years
                        ) / :dt_years
                    )::numeric,
                    2
                )::double precision AS forward_breakeven_bps
            FROM aligned
        ),
        renumbered AS (
            SELECT
            ...
            SELECT
                ...
                CASE
                    WHEN COUNT(*) OVER zw >= 60
                     AND STDDEV_SAMP(forward_breakeven_bps) OVER zw IS NOT NULL
                     AND STDDEV_SAMP(forward_breakeven_bps) OVER zw <> 0
                    THEN ROUND(
                        (
                            (forward_breakeven_bps - AVG(forward_breakeven_bps) OVER zw)
                            / NULLIF(STDDEV_SAMP(forward_breakeven_bps) OVER zw, 0)
                        )::numeric,
                        4
                    )::double precision
                    ELSE NULL
        ...
        display_metrics AS (
            SELECT
                ...
                z_score,
                CASE
                    WHEN LAG(forward_breakeven_bps, 1) OVER (ORDER BY arn) IS NULL THEN NULL
                    ELSE ROUND(
                        (forward_breakeven_bps - LAG(forward_breakeven_bps, 1) OVER (ORDER BY arn))::numeric,
                        2
                    )::double precision
                END AS daily_change_bps,
                CASE
                    WHEN LAG(forward_breakeven_bps, 5) OVER (ORDER BY arn) IS NULL THEN NULL
                    ELSE ROUND(
                        (forward_breakeven_bps - LAG(forward_breakeven_bps, 5) OVER (ORDER BY arn))::numeric,
                        2
                    )::double precision
                END AS weekly_change_bps,
                CASE
                    WHEN LAG(forward_breakeven_bps, 21) OVER (ORDER BY arn) IS NULL THEN NULL
                    ELSE ROUND(
                        (forward_breakeven_bps - LAG(forward_breakeven_bps, 21) OVER (ORDER BY arn))::numeric,
                        2
                    )::double precision
                END AS monthly_change_bps,
                ROUND((MAX(forward_breakeven_bps) OVER tw)::numeric, 2)::double precision AS high_252d_bps,
                ROUND((MIN(forward_breakeven_bps) OVER tw)::numeric, 2)::double precision AS low_252d_bps,
                CASE
                    WHEN MAX(forward_breakeven_bps) OVER tw IS NULL
                      OR MIN(forward_breakeven_bps) OVER tw IS NULL
                      OR MAX(forward_breakeven_bps) OVER tw = MIN(forward_breakeven_bps) OVER tw
                    THEN NULL
                    ELSE ROUND(
                        (
                            (
                                forward_breakeven_bps - MIN(forward_breakeven_bps) OVER tw
                            ) / NULLIF(
                                MAX(forward_breakeven_bps) OVER tw - MIN(forward_breakeven_bps) OVER tw,
                                0
                            ) * 100
                        )::numeric,
                        1
    ...
    return {
        "current_metrics": {
            ...
            "forward_window_label": (
                ...
                f"{start_tenor}{end_tenor}"
            ),
            "forward_breakeven_bps": latest["forward_breakeven_bps"],
            "forward_breakeven_pct": (
                None if latest["forward_breakeven_bps"] is None
                else round(float(latest["forward_breakeven_bps"]) / 100.0, 4)
            ),
            ...
            "start_breakeven_bps": latest["be_short_bps"],
            "end_breakeven_bps": latest["be_long_bps"],
        ...
        "time_series": [
            {
                "date": row["date"],
                "forward_breakeven_bps": row["forward_breakeven_bps"],
                "z_score": row["z_score"],
            }
            for row in rows
        ],
...
def compare_results(
    ...
    # The Python tool's bespoke time_series carries
    # ``forward_breakeven_bps`` + ``z_score`` per row; align to the
    # SQL baseline's same row shape under tighter tolerances.
    row_tolerances = dict(TOLERANCE_BY_FIELD)
    row_tolerances["forward_breakeven_bps"] = TOLERANCE_BY_FIELD["forward_breakeven_bps_row"]
    ...
    mismatches.extend(
        compare_time_series(
            ...
            exact_fields=(),
            numeric_fields=("forward_breakeven_bps", "z_score"),
...
def run_case(
    ...
    nominal_cf, linker_cf, start_tenor, end_tenor = case
    tool_result = calculate_forward_breakeven_simple(
        engine=engine,
...
def assert_pollution_guard(
    ...
    *,
    ...
    """Adversarial probe — verify the polluted leg surfaces zero rows
    AND the Python tool returns a controlled error envelope mentioning
    the right ``instrument_type``.
    """
    failures: List[str] = []
    ...
    # Pollution probe: zero rows on at least the start_tenor (sufficient
    # to refuse the forward).
    with engine.connect() as conn:
        row = conn.execute(
            zero_check_sql,
            {
    ...
    if n_rows != 0:
        failures.append(
            ...
            f"{check_cf} {start_tenor} (pollution probe), but got {n_rows}."
        )
    ...
    if "error" not in tool_result:
        failures.append(
            ...
            f"{start_tenor}→{end_tenor}) — must return controlled error "
            f"envelope.  Got keys: {sorted(tool_result.keys())}"
        )
...
def assert_cross_country_guard(
    ...
    *,
    ...
    country invariant (inherited from the spot primitive) refuses
    the pair and returns a controlled error envelope mentioning both
    ...
    if keys != ["error"]:
        failures.append(
            ...
            f"{start_tenor}→{end_tenor}.  Expected exactly the "
...
def assert_sample_historical_rows(
    ...
    """Independent cross-check on a SAMPLE of historical rows
    (latest, mid-window, earliest) so the SQL/Python parity isn't
    just verified at the snapshot.  ``compare_time_series`` already
    runs a row-by-row comparison; this helper highlights specific
    sample points so a regression on early rows is visible in the
    summary.
    """
    failures: List[str] = []
    ...
    if len(tool_rows) != len(sql_rows):
        ...
        return failures
    # Sample points: first, middle, last.
    ...
    for idx in idxs:
        ...
        if t["date"] != s["date"]:
            ...
            continue
        # Tight tolerance for forward_breakeven_bps at the row level.
        if t["forward_breakeven_bps"] is None and s["forward_breakeven_bps"] is None:
            continue
        if t["forward_breakeven_bps"] is None or s["forward_breakeven_bps"] is None:
            failures.append(
                f"Historical sample[{idx}] @ {t['date']}: "
                f"tool={t['forward_breakeven_bps']} sql={s['forward_breakeven_bps']}"
            )
            continue
        delta = abs(
            float(t["forward_breakeven_bps"]) - float(s["forward_breakeven_bps"])
        )
        if delta > TOLERANCE_BY_FIELD["forward_breakeven_bps_row"] + 1e-12:
            failures.append(
                f"Historical sample[{idx}] @ {t['date']}: "
                f"tool={t['forward_breakeven_bps']} sql={s['forward_breakeven_bps']} "
                f"(delta={delta:.6f})"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the linker forward_breakeven_simple tool against "
            "direct SQL."
        ),
    )
    ...
    print("=" * 80)
    print("LINKER FORWARD BREAKEVEN SIMPLE TOOL — SQL VALIDATION")
    print("=" * 80)
    ...
    for index, case in enumerate(cases, start=1):
        ...
        # Additional sample-historical-row cross-check (first / mid /
        # latest) so a regression on early rows is highlighted in the
        # summary even if compare_time_series already caught it.
        mismatches.extend(
    ...
    swap_result = calculate_forward_breakeven_simple(
        engine=engine,
        params=ForwardBreakevenSimpleInput(
