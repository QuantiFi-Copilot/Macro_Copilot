...
**Phase in flight:** **Phase B core + follow-up + BBG batch ✅ SHIPPED.** Tier 1 progress: A ✅, B-core ✅, B-followup ✅, BBG batch ✅ (substrates for B+ / D / E ATM / E smile), B+/C/D/E tools planned. 367 FX instruments in DB across 5 instrument_types: fx_spot (29), fx_forward (72 = 42 G10 with 2Y/5Y + 30 EM deliverable), fx_ndf (25), fx_vol ATM (85 = 30 G10 + 55 EM/NDF-currency vol), fx_vol_smile (156 = 72 G10 + 84 EM). `calculate_fx_carry` now supports `market_scope` ∈ {G10/EM/ALL} with default G10 (Phase A preserved; explicit opt-in for EM per Codex G2 discipline). All stacked PRs (#178 → #184 → #189 → #191) Open/Ready/mergeable=clean; BBG batch branch local-only.
...
**Immediate next decision:** integrate when ready. Sreeram trusts the work — no formal review-gate. When the merge window opens, integrate in order: #178 → #184 → #189 → #191 → BBG batch branch. With BBG batch shipped, ALL substrates for Phase B+ / Phase D / Phase E (ATM + smile) are now in DB — these can be coded in any order without additional Bloomberg extraction. Phase C (CIP, reads OIS substrate) remains the only-derived (no BBG needed) Tier 1 phase. Phase G (workflows) and Phase H (operators) are infrastructure and can be tackled anytime.
