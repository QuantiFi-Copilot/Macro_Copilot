...
class RealYieldCurveSpreadInput(BaseModel):
    ...
    # resolves the sentinels.  These overrides apply to the SPREAD's
    # own rolling z-score AND the fetch-window buffer math; the inner
    # endpoint level calls intentionally use the YAML default z-score
    # (their z-score is not consumed).  Exposure decisions recorded in
    ...
    z_score_window_days: Optional[int] = Field(
        ...
        description=(
            ...
            "real-yield curve spread (percent) series.  When None "
            "(default), the tool falls through to "
            "``z_score_window_days`` in config.yaml (currently 252 — "
            "1-year window matching every sovereign rates tool and the "
            "linker real_yield_level tool).  Override to 60d / 126d for "
            "tactical / short-horizon framing or 504d for structural-"
            "regime work.  Applies to the spread's own z-score AND the "
            "fetch-window buffer; the inner endpoint level calls use "
            "the YAML default (their z-score is not consumed).  Cross-"
            "config lint enforces DEFAULT alignment; per-call overrides "
            "do not violate the lint.  Exposure decision recorded in "
