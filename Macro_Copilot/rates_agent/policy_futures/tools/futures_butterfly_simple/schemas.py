...
``policy_futures`` domain (ADR 0013) — three strip-position slots on one
...
-----------------------------------------------------
Per the catalog (build_order 17) and ADR 0013, policy futures are
...
class FuturesButterflySimpleInput(BaseModel):
    ...
    strip_position_body, strip_position_wing_long)`` per ADR 0013. The
