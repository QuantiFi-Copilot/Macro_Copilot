...
policy-futures volume + OI snapshot validator

Independently reproduces the policy-futures strip-position volume +
open-interest computation in SQL (current_volume,
current_open_interest, delta_open_interest_1d, rolling 252-day OI
z-score, trailing 252-day OI high / low / percentile, rolling 22-day
volume mean / max, observation_count) and asserts the Python tool
result matches the SQL result row-for-row across a representative
set of (curve_family, strip_position) cases spanning all three V1
policy-futures regimes (SOFR / SONIA / Euribor).

Independence
------------
The SQL baseline filters via ``v_market_data_daily_enriched`` with
the same ``(curve_family, (attributes->>'strip_position')::int,
field_name)`` predicate ``fetch_strip_position`` uses, and INNER-
joins the two series on trade_date (the equivalent of the Python
tool's ``index.intersection`` alignment step). The z-score / range /
percentile / 22d volume rolling stats are computed in SQL via WINDOW
functions; no Python primitive involvement.

Future-anchor guard cross-check
-------------------------------
The runner also calls the Python tool with
``as_of_date = universe_max + 1 day`` (probed against the OI field,
the headline series) and asserts the controlled-error envelope
returns with the documented prefix ("no scoreable strip:"); a
separate SQL probe counts rows past the requested anchor to verify
the guard had a real reason to fire.
...
# The policy_futures playbook universe per ADR 0011 — strip-position-
# keyed only.
...
TOLERANCE_BY_FIELD = {
    # Volume / OI are integer-valued in the source feed but rounded
    # by the Python tool via safe_float; the SQL baseline rounds to 0
...
def choose_test_cases(
    ...
    *,
    ...
    universe that have at least 252 trading days of BOTH PX_VOLUME
    AND OPEN_INT in the lookback buffer — keeps the SQL z-score
...
def sql_baseline(
    ...
    *,
    ...
    functions over the strip-slot's volume + OI history.

    Independence:
      - Filters via ``v_market_data_daily_enriched`` with the same
        ``(curve_family, (attributes->>'strip_position')::int,
        field_name)`` predicate ``fetch_strip_position`` uses.
    ...
    """
    # Pull the master stem (contract_code on instrument_master) so the
    # SQL baseline can surface it on the result. Independent of the
    # Python reference fetcher.
    stem_sql = text(
        """
    ...
    contract_code = stem_row["contract_code"]
    ...
    baseline_sql = text(
        ...
        raw_oi AS (
            SELECT
                trade_date,
                field_value::double precision AS open_interest
            FROM macro_data.v_market_data_daily_enriched
            WHERE curve_family = :curve_family
              AND (attributes->>'strip_position')::int = :strip_position
...
def compare_results(
    ...
    add_exact_field_mismatches(
        ...
        fields=(
            ...
            "contract_code", "observation_count",
        ),
...
def run_case(
    ...
    *,
    ...
    tool_result = calculate_volume_open_interest_snapshot(
        engine=engine,
    ...
    # as_of_date = universe_max + 1 day (probed against the OI field,
    # the headline series) and assert the controlled-error envelope.
    # Independently SQL-probe rows past the requested anchor (must be
    # 0 — proving the guard had a real reason to fire).
    # ------------------------------------------------------------------
    try:
    ...
    guard_result = calculate_volume_open_interest_snapshot(
        engine=engine,
        params=VolumeOpenInterestSnapshotInput(
            curve_family=curve_family,
            strip_position=strip_position,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the policy-futures volume + OI snapshot monitor "
            "against direct SQL."
        ),
    )
    ...
    print("=" * 80)
    print("POLICY-FUTURES VOLUME + OI SNAPSHOT MONITOR — SQL VALIDATION")
    print("=" * 80)
