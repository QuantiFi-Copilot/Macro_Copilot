...
//     endpoint at /api/v1/rates/detail/cpi-surprise + own surfaces; no
...
// Ask: the Stage 5 ``monitor_surface`` + ``ask_surface`` claims were
...
export const MODULE: PrimitiveModuleSpec = {
  ...
  // FM5 — defaults mirror the backend Input: ``country`` is the LLM-facing
  // instrument selector (YAML-locked set US/UK/JP/EU); ``lookback_releases``
  // is the display-window knob (config.yaml default_lookback_releases = 24
  // ≈ 2Y at monthly cadence).  The rolling z window is YAML-locked at 24
  // releases and is NOT a param.
  defaultParams: {
    country: 'US',
