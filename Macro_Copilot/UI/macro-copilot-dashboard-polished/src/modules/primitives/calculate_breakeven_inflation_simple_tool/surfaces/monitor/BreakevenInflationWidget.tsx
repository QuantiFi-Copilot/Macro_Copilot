// ============================================================================
// BreakevenInflationWidget — Monitor bento tile for one bond-implied breakeven.
// ----------------------------------------------------------------------------
...
// counterparty (a breakeven is a same-country object).  Fetches the SAME
// typed-detail endpoint (/api/v1/rates/detail/breakeven) the Build views
// use — the standalone-bridge contract (methodology_exposure.md §5.4).
//
// Per rendering_density.md §8 the Monitor surface is INHERENTLY COMPACT.
// The inflation-compensation caveat surfaces via the kicker + provenance.
// Mirrors the get_real_yield_level_tool RealYieldLevelWidget shape.
// ============================================================================
...
import { pairForLinker } from '../breakevenShared';
...
export function BreakevenInflationWidget({ params }: Props) {
  const fetchParams = useMemo<BreakevenDetailParams | null>(() => {
  ...
  const { data, error, isLoading } = useFetchDetailBreakeven(fetchParams);
  ...
  const pairLabel = pair ? `${pair.country} · ${pair.nominalShort}/${pair.linkerShort}` : m.linker_curve_family;
          ...
          <div className="mt-1 flex items-center gap-2 font-mono text-[9.5px] text-fg-faint">
            ...
            <div className="relative h-1.5 flex-1 rounded-full bg-white/[0.04] ring-1 ring-line-subtle">
              {(() => {
                ...
                const pct = ((m.breakeven_bps - m.low_252d_bps) / range) * 100;
                return (
                  <span
                    aria-hidden
...
function useFetchDetailBreakeven(params: BreakevenDetailParams | null) {
