...
2. **Workflow-bridge artifact shapes** (which output shapes the workflow executor can lift into typed artifacts for composition with operators). The bridge (`shared/artifacts/adapters/` + `shared/workflow/executor.py`) dispatches two artifact shapes: `Series` (from a primitive's canonical `TimeSeries` payload) and `Panel` (multi-series outputs declared via `output_artifact_type = "Panel"`). Within the `Series` shape, a leaf may select **any declared output series**, including a **named component of a multi-component fit** — see the contract below. Outputs that are neither — categorical outputs, and scalar/snapshot statistical fits (`half_life`, `yield_change_attribution_pca`) — are valid primitives standalone but are **not bridge-composable** (they are `TERMINAL_ONLY_SNAPSHOT` in the composability audit; reachable on the direct-fetch lane only).
...
### Selectable output series — the multi-component contract (STANDARD)

A primitive may expose **more than one** selectable time-series, and the open-DAG composer/selector picks ONE per leaf via `output_field`. This is the single, uniform mechanism for both shapes of multi-output primitive — there is no per-tool code; everything is driven by the declaration in `PrimitiveSpec.output_field_units`.

- **Top-level secondary fields.** A primitive may declare several canonical `time_series_*` fields (e.g. `curve_spread` → `time_series_spread` + `time_series_zscore`; `rolling_regression` → `time_series_betas` / `time_series_alpha` / `time_series_residual` / `time_series_r_squared`). Each is a `TimeSeries`-typed field on the `<Tool>Output` class, declared in `output_field_units`. A leaf binds the one it needs via `chosen_output_field` (e.g. `output_field="time_series_residual"`).
- **Components of a `List[TimeSeries]` fit field.** A statistical fit that emits one series per component (e.g. `pca_yield_curve` → `time_series_factors: List[TimeSeries]`, each `series_name='<curve>_pc<k>_factor_<freq>'`) declares **each component key** in `output_field_units` (PCA → `pc1` / `pc2` / `pc3`). The Series bridge resolves `output_field="pc1"` by token-matching the element's `series_name` (`_resolve_list_component_series` in `shared/artifacts/adapters/from_time_series.py`). PC1 thus becomes a normal `Series` leaf — no SeriesSet, no extraction operator. The selector's catalogue filter (`orchestrator/selectors.py`) keeps these component keys because the `<Tool>Output` class declares a `List[TimeSeries]` field.

**How to add a multi-output primitive (the standardized path):**
1. Declare every selectable series in `output_field_units` — a `TimeSeries` field name, OR (for a `List[TimeSeries]` fit) each component key (matched against the element `series_name`).
2. Nothing else: the bridge, the selector catalogue, the validator (`shared/workflow/validate.py` gates `output_field` against `output_field_units`), and the composability audit all read that one declaration.
3. The composability audit (`tests/orchestrator/open_dag/test_composability_audit.py`) fails the build on `UNDECLARED`; `tests/orchestrator/open_dag/test_multicomponent_output_fields.py` locks the component contract.

**Known limits / left behind (read before extending):**
- **PCA component count.** Only `pc1`/`pc2`/`pc3` are declared (matching the default `n_components=3`). A query that fits MORE components must add the extra keys to `output_field_units`; until then an undeclared `pc4` is refused cleanly by the validator (never mis-bound). This is the deliberate, safe default.
- **Whole-set operators.** The current mechanism extracts ONE component as a `Series`. If an operator ever needs the WHOLE component set at once (e.g. a cross-component rank), bridge the fit as a first-class `SeriesSet` (`output_artifact_type="SeriesSet"` + a new `tool_output_to_artifact_series_set` adapter + a `BRIDGEABLE_SERIES_SET` audit class) and let the composer wire `select_from_series_set`. This is a tracked extension, not needed for single-component selection.
- **Scalar/snapshot fits.** `half_life`, `yield_change_attribution_pca`, and the scanners are `TERMINAL_ONLY_SNAPSHOT` (no time-series to select) — direct-fetch lane only, by design.
