"""
test_real_yield_level_parity.py — Snapshot parity tests for
                                  get_real_yield_level
============================================================

Locks in the *real production* output of ``get_real_yield_level`` for
three representative linker pillars (USD_TIPS 10Y 365d, GBP_LINKER 10Y
730d, EUR_FR_LINKER 5Y 365d).  Any commit that changes the math —
directly or through a primitive in ``shared/analytics/`` or a YAML edit
to the bundled ``config.yaml`` — must keep these tests green or come
...
``tests/fixtures/real_yield_level_v1/_capture.py``; the test itself
runs fully offline by replaying captured ``raw_rows`` through a mocked
fetcher.

How it works
------------
For each fixture in ``tests/fixtures/real_yield_level_v1/``:
...
2. Reconstruct the long-format DataFrame the tool's DB fetcher would
   return from ``input.raw_rows``.  ``fetch_single_tenor`` returns only
   ``[trade_date, field_value]`` for this primitive, so the replayed
   frame matches that shape exactly.
3. Patch ``rates_agent.inflation_indexed_bonds.tools.real_yield_level.compute.fetch_single_tenor``
   ...
   ``get_real_yield_level`` and does NOT propagate ``compute.py``'s
   imports — see the package init's "Note for tests" section.
4. Build a ``RealYieldLevelInput`` from the recorded params and call
   ``get_real_yield_level(engine=None, params=...)``.
...
----------------------------------------
``get_real_yield_level`` rounds every numeric output via the
``yield_round_decimals`` / ``z_score_round_decimals`` /
...
across pandas / numpy versions — anything bigger than that is real math
drift and the test fails.

Regenerating fixtures
---------------------
Only after a deliberate methodology change.  See
``tests/fixtures/real_yield_level_v1/README.md``.
"""
...
FIXTURES_DIR = Path(__file__).parent / "fixtures" / "real_yield_level_v1"
...
def _canonicalise_rows(rows: list[dict]) -> str:
    ...
    here would make every parity run fail with a hash error.
    """
...
@pytest.mark.skipif(
    ...
    reason=(
        ...
        "`python tests/fixtures/real_yield_level_v1/_capture.py` against "
        "the live DB to generate."
    ),
)
@pytest.mark.parametrize("fixture_path", _FIXTURE_PATHS, ids=_FIXTURE_IDS)
def test_real_yield_level_parity(fixture_path: Path) -> None:
    """Run the captured inputs through ``get_real_yield_level`` with the
    DB fetcher mocked and the date frozen, and assert byte-equal output
    ...
    # package init.  The package init re-exports get_real_yield_level
    # but does NOT propagate compute.py's imports (fetch_single_tenor,
    # date) into its own namespace, so patching the package init would
    # be a no-op.  See
    # rates_agent/inflation_indexed_bonds/tools/real_yield_level/__init__.py
    # "Note for tests" docstring section.
    target_module = (
        "rates_agent.inflation_indexed_bonds.tools.real_yield_level.compute"
    )
    with patch(f"{target_module}.fetch_single_tenor", return_value=raw_df), \
         patch(f"{target_module}.date", _FrozenDateForRealYieldLevel):
        # engine is unused because fetch_single_tenor is mocked.
        actual = get_real_yield_level(engine=None, params=params)
