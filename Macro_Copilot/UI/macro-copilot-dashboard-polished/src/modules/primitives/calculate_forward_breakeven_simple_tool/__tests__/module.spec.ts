...
// ============================================================================
// src/modules/primitives/calculate_forward_breakeven_simple_tool/__tests__/module.spec.ts
// ----------------------------------------------------------------------------
...
const FOLDER = 'calculate_forward_breakeven_simple_tool';
...
check('default linker_curve_family is a recognised same-country pair', () => {
  const linker = MODULE.defaultParams?.linker_curve_family;
  ...
  if (typeof linker !== 'string' || !RECOGNISED.has(linker)) {
    throw new Error(
      `defaultParams.linker_curve_family must be a recognised linker family; got ${JSON.stringify(linker)}.`,
    );
  }
});

check('default forward_pair has matching start_tenor + end_tenor', () => {
  // The backend's tenor-mode input requires (start_tenor, end_tenor)
  // together with start_years < end_years; the schema layer rejects
  // partial/inverted inputs.  defaultParams must carry both so the very
  // first render against the typed-detail endpoint succeeds without an
  // interim "missing input" error.
