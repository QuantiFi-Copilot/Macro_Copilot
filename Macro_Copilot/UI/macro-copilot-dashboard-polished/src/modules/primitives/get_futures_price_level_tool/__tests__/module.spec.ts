...
// ============================================================================
// src/modules/primitives/get_futures_price_level_tool/__tests__/module.spec.ts
// ----------------------------------------------------------------------------
...
// rendering-density contract checks (rendering_density.md §1) and the
// catalog-23 monitor-widget-id collision-avoidance check.
// ============================================================================

import { assertStandardModuleInvariants } from '../../../__test-utils';
import { MODULE } from '../module';

const FOLDER = 'get_futures_price_level_tool';
...
check('claims monitor_surface tier', () => {
  if (!MODULE.tiers.includes('monitor_surface')) {
    throw new Error(
      `tiers missing 'monitor_surface'; catalog entry required_tiers includes monitor_surface per the desk-canonical bond-futures eligibility.  Got tiers=${JSON.stringify(MODULE.tiers)}`,
    );
  }
});
...
check('monitorWidgets carries the Bond Futures Price Level tile', () => {
  ...
  if (!ids.includes('bond_futures_price_level')) {
    throw new Error(
      `monitorWidgets missing the 'bond_futures_price_level' entry; got ids=${JSON.stringify(ids)}.`,
    );
  }
  if (ids.includes('policy_futures_price_level')) {
    throw new Error(
      "bond_futures monitor widget MUST NOT reuse the 'policy_futures_price_level' id — that id is held by the policy_futures cousin.  Per catalog-23 collision watch, use 'bond_futures_price_level'.",
    );
  }
});
