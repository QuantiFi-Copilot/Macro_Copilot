...
only shape difference is start/end tenor naming on the input and
the dual-compounding geometric forward formula at the compose
layer.  The seam topology is identical — all seams on the INNER
level primitive's compute module:
...
from rates_agent.inflation_swaps.tools.inflation_swap_forward.compute import (  # noqa: E402
    CONFIG_PATH as _ISF_CONFIG_PATH,
)
...
_CASES: list[dict] = [
    {
        "fixture_name": "usd_zcis_5y10y_365d",
        "params": {
    ...
    {
        "fixture_name": "eur_zcis_5y10y_365d",
        "params": {
    ...
    {
        "fixture_name": "gbp_zcis_5y10y_365d",
        "params": {
...
def _capture_one(case: dict, engine, captured_at: str, db_name: str) -> dict:
    params = InflationSwapForwardInput(**case["params"])
    frozen_today = date.today()

    cfg = load_tool_config(_ISF_CONFIG_PATH)
    ...
    for tenor in (params.start_tenor, params.end_tenor):
        df = fetch_zcis_single_pillar(
            engine=engine, curve_family=params.curve_family,
            tenor=tenor, field_name=field_name_resolved,
        ...
        raw_rows[
            _series_key(
                curve_family=params.curve_family, tenor=tenor,
    ...
    expected_output = calculate_inflation_swap_forward(
    ...
    return {
        ...
        "tool_function": "calculate_inflation_swap_forward",
        "capture": {
...
def main() -> None:
    ...
    for case in _CASES:
        ...
        print(
            ...
            f"forward={m['forward_zcis_bps']:+.2f}bps  "
