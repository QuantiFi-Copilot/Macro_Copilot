...
ADR 0011 V1 refusal returns a ``{"error": "..."}`` envelope
mentioning ``delivery_month_type`` + ``ADR 0011``. The MCP layer's
...
# The two clean-built families per ADR 0011 V1. EUR_SHORT_RATE_FUT
...
def run_eur_refusal_case(engine, *, lookback_days: int, field_name: str) -> List[str]:
    """Layer B refusal probe — confirms the ADR 0011 V1
    NotImplementedError fires against the live DB stack and that the
    error message names both ``delivery_month_type`` and ``ADR 0011``."""
    mismatches: List[str] = []
    params = FuturesPackAverageSimpleInput(
    ...
    if "ADR 0011" not in message:
        mismatches.append(
            "EUR_SHORT_RATE_FUT refusal probe — message missing "
            f"``ADR 0011``: {message!r}"
        )
...
def main() -> None:
    ...
    print_case_header(
        ...
        "EUR_SHORT_RATE_FUT refusal (ADR 0011 V1)",
    )
