"""
test_cross_country_breakeven_spread_simple_parity.py — Snapshot
parity tests for calculate_cross_country_breakeven_spread_simple
==================================================================

PR15 backfill — modelled on
``tests/test_breakeven_curve_spread_parity.py``; the only shape
difference is that the two inner spot calls use DIFFERENT
(nominal, linker) pairs (one per country) at the SAME shared
tenor.
...
``tests/fixtures/cross_country_breakeven_spread_simple_v1/README.md``.
"""
...
FIXTURES_DIR = (
    ...
    / "cross_country_breakeven_spread_simple_v1"
)
FLOAT_ABS_TOL = 1e-9
...
def test_cross_country_breakeven_spread_simple_parity(
    fixture_path: Path,
) -> None:
    ...
    params = CrossCountryBreakevenSpreadSimpleInput(**fx["input"]["params"])
    ...
    with patch(
        ...
        actual = calculate_cross_country_breakeven_spread_simple(
