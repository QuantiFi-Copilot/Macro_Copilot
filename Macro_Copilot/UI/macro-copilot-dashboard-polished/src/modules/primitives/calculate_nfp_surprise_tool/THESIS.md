...
* **Build (generic builder).** Controls rail generated from the backend `ToolCard.input_fields` (country fixed to US, event_type, window length).  Output canvas renders the per-release surprise series via `AutoRenderer`.
* **Ask (generic research card).** Once the Supervisor routes NFP-surprise queries to the sovereign_bonds domain (separate orchestration session), the 7-zone `AssistantResearchCard` renders the result with full chart + provenance + follow-ups.
* **Monitor.** Not surfaced — NFP is an event-class tool (one fire per release), not desk-glanceable state.  Excluded by the surface contract's Monitor eligibility rule.

## 3. Why these surfaces and not others?

NFP is a monthly-release event read on the first Friday of every month.  The desk reads "what's the latest print" episodically, not as continuously-tracked board state.  That puts it firmly in the Ask + Build affordances, NOT the Monitor surface.  A bespoke Build canvas or Ask card would only add value if there were per-tool framing the generic surfaces miss; today there isn't — the generic schema-driven builder + 7-zone research card cover the user's read perfectly.

## 4. What would change the design?

- Bespoke chart layout (e.g. per-component employment decomposition: private vs government, hours worked, average earnings overlay) → claim a capability tier and ship a per-tool surface file inside this folder.
- Per-tool framing on the Ask card that adds value over the generic 7-zone treatment (e.g. a structured header callout showing "surprise: +120k jobs · z=2.1" above the standard zones) → claim a capability tier and ship a per-tool surface that EXTENDS the generic, not replaces it.
