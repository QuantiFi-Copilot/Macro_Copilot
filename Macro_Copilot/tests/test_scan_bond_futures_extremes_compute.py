...
Covers:
  ...
  7. Methodology disclosure: present on EVERY row AND on the response,
     ...
     wide-sweep + non-DV01-spread + ADR 0011 caveats.
...
# SOFR_FUT / SONIA_FUT / EUR_SHORT_RATE_FUT per ADR 0011 V1 scope).
_DEFAULT_STEMS = [
    ("UST_FUT", "TY1", "10Y"),
    ("UST_FUT", "UXY1", "10Y"),
    ("UST_FUT", "US1", "30Y"),
...
class TestBundledConfig:
    ...
    def test_convention_defaults(self):
        ...
        # Closed-family per ADR 0011 V1 — sovereign-bond futures only.
...
class TestMethodologyDisclosure:
    def test_disclosure_present_on_response_and_every_row(self):
        ...
        assert "ADR 0011" in md_response or "Phase-4" in md_response
...
class TestSchemaBehaviour:
    ...
    def test_policy_futures_curve_family_refused(self):
        ...
        ADR 0011 V1 closed-family whitelist."""
        from pydantic import ValidationError
