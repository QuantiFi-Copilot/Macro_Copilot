...
// ============================================================================
// src/modules/primitives/calculate_cross_country_breakeven_spread_simple_tool/__tests__/module.spec.ts
// ----------------------------------------------------------------------------
...
const FOLDER = 'calculate_cross_country_breakeven_spread_simple_tool';
...
check('claims monitor_surface tier', () => {
  if (!MODULE.tiers.includes('monitor_surface')) {
    throw new Error(
      `tiers missing 'monitor_surface'; this tool must claim it per the catalog entry's required_tiers.  Got tiers=${JSON.stringify(MODULE.tiers)}`,
    );
  }
});
...
check('monitorWidgets declares the cross-country breakeven tile', () => {
  const widgets = MODULE.monitorWidgets ?? [];
  const id = 'cross_country_breakeven_spread';
  if (!widgets.some((w) => w.id === id)) {
    throw new Error(
      `monitorWidgets missing id='${id}'; the catalog claims monitor_surface and the module spec must enumerate exactly that widget.  Got widget ids=${JSON.stringify(widgets.map((w) => w.id))}`,
    );
  }
});
