"""
compute.py — Config-driven same-country breakeven curve spread
==============================================================

Fourth tool in the ``inflation_indexed_bonds`` domain.  Owns the
desk concept of a same-country breakeven curve spread / inflation-
compensation term structure between two breakeven tenors of the
same nominal/linker pair (e.g. UST/USD_TIPS 2s10s breakeven,
UK_GILT/GBP_LINKER 5s30s breakeven).  Computed per-trade-date via
the SPREAD formula:

    spread_bps = long_breakeven_bps - short_breakeven_bps

where ``short_breakeven_bps`` and ``long_breakeven_bps`` are the
spot bond-implied breakevens at ``short_tenor`` and ``long_tenor``
respectively (each computed by
``calculate_breakeven_inflation_simple``).  T_short / T_long are
...
Concept honesty
---------------
This tool returns the TERM STRUCTURE of bond-implied INFLATION
COMPENSATION, NOT the term structure of pure expected inflation.
Each endpoint breakeven carries an inflation risk premium and a
relative liquidity premium between the nominal sovereign and the
linker; the curve spread inherits both at each endpoint.  The
...
gets two load-bearing guards "for free":
  ...
  - Same-country / same-currency invariant — i.e. the
    ...
    any market-data SELECT (DESIGN_PRINCIPLES.md §5 / STANDARD_TOOL_AND_YAML_RULES.md §H).

Both guards run on EACH inner endpoint call; if either endpoint's
spot breakeven returns its own controlled error envelope, this
tool surfaces a controlled error envelope of its own that names
which endpoint failed and propagates the inner error context.

The spot primitive's compute() is invoked with ``config=`` set
...
automatically see those overrides flow through to the inner spot
calls.
...
latest aligned trade_date).  Our rolling z-score / trailing range
...
trim ourselves.  The buffer math matches the spot primitive's
...
------------------------
The two endpoint breakeven series are inner-joined on date so we
only emit rows where BOTH endpoints had data — matches the
forward primitive's outer alignment step.  No ffill across the
...
``low_252d_bps``, ``percentile_252d``) embed the number;
changing the convention without renaming the wire fields would
silently lie about what the percentile is computed against.
``compute()`` raises ``NotImplementedError`` if this is set to
anything else; see ``methodology.planned_extensions`` for the
path to making it configurable.
...
fetch start-date logic lives entirely inside the spot
primitive, which already exposes its own date seam.
"""
...
def _conventions_from_config(config: ToolConfig) -> dict:
    """Pull the methodology kwargs the breakeven_curve_spread
...
def calculate_breakeven_curve_spread(
    engine: Engine,
    params: BreakevenCurveSpreadInput,
    config: Optional[ToolConfig] = None,
) -> Dict[str, Any]:
    """Calculate the same-country breakeven curve spread between
    two tenors of the same nominal/linker pair.
    ...
    -------
    dict
        Serialised ``BreakevenCurveSpreadOutput``, or
    ...
    # time_series_breakeven to its own ``lookback_days`` of
    # display history.  We need a longer window so OUR rolling
    # stats are fully populated from the first row of OUR display
    # window; match the buffer math the spot primitive uses
    # internally so we don't double-buffer.
    # ------------------------------------------------------------------
    ...
    # invariant (instrument_master (country, currency) check
    # before any market-data SELECT) for free.  We pass ``config=``
    # so the inner conventions use the same values this primitive
    # uses; cross-config lint enforces value-agreement on the
    # bundled YAMLs, and any test override on this primitive's
    # ToolConfig flows through to the inner spot calls.
    # ------------------------------------------------------------------
    ...
    if "error" in short_inner:
        return {
            "error": (
                f"Breakeven curve spread "
                f"{params.short_tenor}{params.long_tenor} could not "
                f"be computed for {params.nominal_curve_family} vs "
                f"{params.linker_curve_family}: the short-tenor "
    ...
    if "error" in long_inner:
        return {
            "error": (
                ...
                f"{params.short_tenor}{params.long_tenor} could not "
                f"be computed for {params.nominal_curve_family} vs "
                f"{params.linker_curve_family}: the long-tenor "
    ...
    # so we only emit rows where BOTH endpoints had data — no
    ...
    if not short_rows or not long_rows:
        return {
            "error": (
                f"Breakeven curve spread "
                f"{params.short_tenor}{params.long_tenor} could not "
                "be computed: at least one endpoint returned no "
                "canonical TimeSeries rows.  This is a bug — the "
                "spot primitive returned a snapshot but no history.  "
                "Please report with the curve_family pair "
    ...
    short_df = pd.DataFrame(short_rows).rename(
        columns={"value": "short_breakeven_bps"},
    )
    long_df = pd.DataFrame(long_rows).rename(
        columns={"value": "long_breakeven_bps"},
    )
    ...
    if wide.empty:
        return {
            "error": (
                f"After aligning the two endpoint breakeven series "
                f"for {params.nominal_curve_family} vs "
                f"{params.linker_curve_family} "
                f"({params.short_tenor} → {params.long_tenor}), no "
                "overlapping dates remain.  The two endpoint tenors "
                "may not have a common date history at this "
                "country — verify both curves have data at both "
                "tenors."
            )
        }

    # ------------------------------------------------------------------
    # Spread formula:  spread_bps = long - short
    ...
    # discipline as compute_spread_bps in the sovereign curve_spread
    # primitive.
    # ------------------------------------------------------------------
    wide["spread_bps"] = (
        wide["long_breakeven_bps"] - wide["short_breakeven_bps"]
    ...
    # / forward_breakeven_simple / real_yield_level / OIS rate_level.
    # ------------------------------------------------------------------
    ...
    if display_df.empty:
        return {
            "error": (
                f"No breakeven curve spread observations within the "
    ...
    # Trailing high/low/percentile of the breakeven curve spread
    # (bps scale).
    ...
    metrics = BreakevenCurveSpreadCurrentMetrics(
    ...
    output = BreakevenCurveSpreadOutput(
...
def _series_name_slug(
    *,
    ...
    ``<nominal>_<linker>_<shortten>_<longten>``.
    """
    return (
...
def _build_canonical_spread_series(
    ...
    *,
    ...
    return TimeSeries(
        series_name=f"{slug}_breakeven_spread",
        units=TimeSeriesUnits.BPS,
        description=(
            f"Same-country breakeven curve spread "
            f"({nominal_curve_family}-{linker_curve_family}) "
            f"{short_tenor}/{long_tenor} (long_breakeven_bps - "
            "short_breakeven_bps) over the displayed window.  Term "
            "structure of inflation compensation; not the term "
            "structure of pure expected inflation (carries "
            "inflation risk premium and liquidity premium at each "
            "endpoint)."
        ),
        rows=rows,
    )
...
def _build_canonical_zscore_series(
    ...
    *,
    ...
    return TimeSeries(
        series_name=f"{slug}_breakeven_spread_zscore",
        units=TimeSeriesUnits.Z_SCORE,
        description=(
            f"Rolling z-score of the {nominal_curve_family}-"
            f"{linker_curve_family} breakeven curve spread "
            f"(in bps) over {short_tenor}/{long_tenor} vs its own "
            "trailing window."
        ),
        rows=rows,
    )
