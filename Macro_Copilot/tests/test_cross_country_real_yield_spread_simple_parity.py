"""
test_cross_country_real_yield_spread_simple_parity.py — Snapshot
parity tests for calculate_cross_country_real_yield_spread_simple
==================================================================

PR15 backfill — modelled on
``tests/test_real_yield_curve_spread_parity.py``; the only shape
differences are TWO identity-guard rows per scenario (one per
linker leg in the compose primitive's OWN compute module) and TWO
inner endpoint fetches in DIFFERENT curve_families instead of two
tenors on a single curve.
...
``tests/fixtures/cross_country_real_yield_spread_simple_v1/README.md``.
"""
...
FIXTURES_DIR = (
    ...
    / "cross_country_real_yield_spread_simple_v1"
)
FLOAT_ABS_TOL = 1e-9
...
def test_cross_country_real_yield_spread_simple_parity(
    fixture_path: Path,
) -> None:
    ...
    params = CrossCountryRealYieldSpreadSimpleInput(**fx["input"]["params"])
    ...
    compose_module = (
        ...
        "cross_country_real_yield_spread_simple.compute"
    )
    level_module = (
        "rates_agent.inflation_indexed_bonds.tools."
    ...
    with patch(
        ...
        actual = calculate_cross_country_real_yield_spread_simple(
