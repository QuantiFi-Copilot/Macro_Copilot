// ============================================================================
// surfaces/BuildExtended.tsx — Extended Build view for ``scan_extremes_tool``.
// SCANNER shape (sovereign yield universe).
// ----------------------------------------------------------------------------
...
// universe-scan canvas mounted by ``VirtualPrimitiveCanvas`` for the legacy
// typed-view dispatch path AND by the multi-tool DAG modal infrastructure when
// the compact card calls ``onExpand``.  The scanner's headline data is a
// RANKED LIST, not a time series — this component owns its own scanner-shaped
// layout (identity row → scan-summary callouts → controls strip → distribution
// histogram → ranked detail table → methodology card → lineage footer) and
// reuses the shared ``ControlsStrip`` / ``MethodologyCard`` / ``LineageFooter`` /
// ``FreshnessPill`` elements so the chrome matches the rest of the Build
// catalogue.
//
// Design reference: ./mockups/Extended.png.
// Sibling shape: ../../scan_inflation_swaps_extremes_tool/surfaces/BuildExtended.tsx.
// ============================================================================
...
import {
  SCAN_EXTREMES_COMPACT_CAVEAT,
  SOVEREIGN_SCANNER_CURVE_OPTIONS,
  buildHistogramBins,
  ...
  instrumentCurveShort,
  instrumentFlag,
  parseScanSummary,
  sovereignFamilyFor,
  useScanExtremes,
  zScoreTone,
} from './scanExtremesShared';
import type { BuildExtendedProps } from '@/components/shared/build';
import type { ScanExtremesResultRow } from '@/types/rates';

// ---------------------------------------------------------------------------
// Defaults — the controls strip and the fetch hook share one source of
// truth.  The backend resolves None ⇒ YAML default; the extended view
// surfaces the YAML default as the displayed value and passes the number
// through to the hook so user overrides round-trip via URL state.
// ---------------------------------------------------------------------------
...
const CURVE_FAMILY_OPTIONS: ReadonlyArray<{ value: string; label: string }> = [
  { value: ALL_CURVE_FAMILIES_VALUE, label: 'All sovereigns (G10)' },
  ...SOVEREIGN_SCANNER_CURVE_OPTIONS,
];
...
const BuildExtended: React.FC<BuildExtendedProps> = ({
  ...
  const { data, isLoading, errorMessage } = useScanExtremes({
    curveFamilies,
    topN: Number(topNStr),
  ...
  return (
    <div
      ...
      data-testid="sovereign-scanner-extended"
    >
      {/* ---------- Title section ---------- */}
      <section className="grid grid-cols-1 gap-4 border-b border-line-subtle px-6 pt-5 pb-5 lg:grid-cols-[1fr_auto]">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[11px] uppercase tracking-wide text-fg-muted">
            ...
            <span className="font-semibold">G10 SOVEREIGN UNIVERSE SCAN</span>
          ...
          <div className="flex flex-wrap items-center gap-2 text-[12px] text-fg-secondary">
            {summary.scanned != null && (
              <span>{summary.scanned} sovereign instruments scanned</span>
            )}
      ...
      <div className="grid grid-cols-1 gap-3 px-6 pt-4 lg:grid-cols-3">
        <SummaryCallout
          ...
          count={highCount}
          hint="Stems where the 252d-rolling yield sits above its trailing-year mean by ≥ threshold."
        />
        <SummaryCallout
          tone="extreme-down"
          label="Biggest moves (low)"
          count={lowCount}
          hint="Stems where the 252d-rolling yield sits below its trailing-year mean by ≥ threshold."
        />
        <SummaryCallout
          tone="elevated"
...
function CoverageCard({
  ...
  // Surface the country coverage of the flagged extremes — the desk sees
  // WHICH G10 curves showed up in the top-N so the "is this just one
  // country?" question is answered at a glance.
  const countries = new Set<string>();
  for (const r of rows) {
    const meta = sovereignFamilyFor(r.curve_family);
    if (meta) countries.add(meta.countryCode);
  }
  return (
    <div className="card flex h-full flex-col gap-2 px-4 py-3">
      <span className="kicker text-fg-muted">CURVE COVERAGE</span>
...
function RankedTable({
  ...
  isLoading?: boolean;
  errorMessage?: string;
  onOpenInBuild: (row: ScanExtremesResultRow) => void;
}) {
  return (
        ...
        <table className="w-full min-w-[760px] text-[12.5px]">
          <thead>
            <tr className="border-b border-line-subtle text-left text-[10.5px] uppercase tracking-wide text-fg-muted">
              ...
              <th className="px-3 py-2 text-right font-normal">Yield</th>
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-1.5 text-fg-primary">
                        ...
                        <span>{instrumentCountryLabel(row)}</span>
                      </div>
                    </td>
                    <td className="px-3 py-2">
                      <span className="font-mono text-[12px] text-fg-secondary">
                        {instrumentCurveShort(row)}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-2 text-right font-mono tabular-nums text-fg-primary">
                      {formatYieldPct(row.current_yield_pct)}
                    </td>
