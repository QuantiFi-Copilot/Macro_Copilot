...
- **Not a page-organised codebase (any longer).** As of [`../05_decisions/0014-frontend-module-architecture.md`](../05_decisions/0014-frontend-module-architecture.md), code is organised by **module** (one folder per backend unit), not by page. The page folders that remain (`build/`, `library/`, `monitor/`, `ask/`, `layout/`) host modules' surfaces; they do not own primitive-specific logic.
