...
V1 monitor for the ``policy_futures`` domain (ADR 0013) — universe-
...
- ``short_rate_regime`` is the per-row RFR-vs-IBOR disclosure
  required by ADR 0013. SOFR_FUT and SONIA_FUT rows carry
...
# admitted by the V1 catalog (ADR 0013). Adding a new family
...
class ScanPolicyFuturesExtremesInput(BaseModel):
    ...
    curve_families: Optional[List[str]] = Field(
        ...
        description=(
            ...
            "ADR 0013 — passing a bond-futures curve (UST_FUT / "
...
class ScanPolicyFuturesExtremesResultRow(BaseModel):
    ...
    disclosure (ADR 0013) so a desk consumer copying ONE row out
    ...
    short_rate_regime: Literal["RFR", "IBOR"] = Field(
        ...,
        description=(
            "Per-row short-rate regime disclosure (ADR 0013). "
    ...
    methodology_disclosure: str = Field(
        ...
        min_length=1,
        description=(
            "P5 / ADR 0013 / catalog-guardrail disclosure — "
...
class ScanPolicyFuturesExtremesOutput(BaseModel):
    ...
    Wire shape:
      ...
      - ``methodology_disclosure``: the P5 / ADR 0013 caveat
    ...
    methodology_disclosure: str = Field(
        ...
        min_length=1,
        description=(
            "P5 / ADR 0013 / catalog-guardrail disclosure carried "
