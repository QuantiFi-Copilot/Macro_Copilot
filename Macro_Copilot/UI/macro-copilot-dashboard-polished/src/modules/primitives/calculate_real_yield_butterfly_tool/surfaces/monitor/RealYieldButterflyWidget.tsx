// ============================================================================
// RealYieldButterflyWidget — Monitor bento tile for one linker real-yield
// butterfly (3-point curvature on a single linker curve).
// ----------------------------------------------------------------------------
// Parameterised on (curve, triplet, lookback_days) where ``curve`` is the
// linker curve_family and ``triplet`` is the registered (short, belly, long)
// tuple selected by label (e.g. "5s10s30s").  Fetches the SAME typed-detail
// endpoint (/api/v1/rates/detail/real-yield-butterfly) the Build views use
...
// The curvature-of-real-yields framing surfaces via the kicker; the
// linker caveat lives in the methodology card on the Extended view.
// Mirrors the BreakevenButterflyWidget shape, adapted for a single-curve
// input (no nominal counterparty).
// ============================================================================
...
export function RealYieldButterflyWidget({ params }: Props) {
  const fetchParams = useMemo<RealYieldButterflyDetailParams | null>(() => {
    // ``curve`` is the linker family; fall back to an explicit
    ...
    const meta = curveForFamily(curveFamily);
    if (!meta) return null;

    // The triplet selector value is the registered triplet label (e.g.
    // "5s10s30s").  Look up the (short, belly, long) tuple from the
    ...
    const triplets = REAL_YIELD_BUTTERFLY_TRIPLETS_BY_CURVE[curveFamily] ?? [];
    const selected =
  ...
  const { data, error, isLoading } = useFetchDetailRealYieldButterfly(fetchParams);
  ...
  // Display in bps (×100); backend ships current_butterfly_pct in PERCENT.
  const flyBps = m.current_butterfly_pct != null ? m.current_butterfly_pct * 100 : 0;
  const highBps = m.high_252d_pct != null ? m.high_252d_pct * 100 : null;
  const lowBps = m.low_252d_pct != null ? m.low_252d_pct * 100 : null;
  const dailyNeg = (m.daily_change_bps ?? 0) < 0;
  const zAbs = Math.abs(m.current_z_score ?? 0);
  const curve = curveForFamily(m.curve_family);
  ...
  const curveLabel = curve
    ? `${curve.country} · ${m.curve_family}`
    : m.curve_family;
...
function useFetchDetailRealYieldButterfly(
  params: RealYieldButterflyDetailParams | null,
) {
