...
V1 monitor #2 for the ``policy_futures`` domain (ADR 0011) — daily
...
-----------------------------------------------
Per the catalog (build_order 15) and ADR 0011, policy futures are
...
class VolumeOpenInterestSnapshotOutput(BaseModel):
    ...
    methodology_disclosure: str = Field(
        ...
        min_length=1,
        description=(
            "P5 / ADR 0011 caveat carried on every response, including "
