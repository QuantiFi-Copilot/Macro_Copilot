"""
compute.py — Config-driven same-curve ZCIS butterfly
======================================================

Owns the desk concept of a *same-curve zero-coupon inflation swap
(ZCIS) butterfly* (3-point ZCIS curve curvature) on a single ZCIS
curve family (e.g. USD_ZCIS 5s10s30s, EUR_ZCIS 2s5s10s, GBP_ZCIS
2s10s30s).  Computed per-trade-date via the FIXED simple-butterfly
weighting:
...
where ``short_zcis_pct`` / ``belly_zcis_pct`` / ``long_zcis_pct``
are the ZCIS rate levels at ``short_tenor`` / ``belly_tenor`` /
``long_tenor`` respectively (each computed by
``calculate_inflation_swap_rate_level``).  T_short / T_belly /
T_long are parsed from the tenor strings via the shared
...
date butterfly.  That gets the load-bearing four-conjunct SELECT
guard "for free":
...
Both filters are enforced inside the level primitive's compute().
Composing three level calls means all three legs inherit the
four-conjunct guard.
...
conventions on the butterfly primitive's ToolConfig automatically
see those overrides flow through to the inner level calls.

Same-curve invariant
--------------------
The three endpoint reads MUST share ``inflation_index_family``,
...
-----------------------------
The butterfly arithmetic is performed on raw ZCIS par rates only —
NO subtraction of a model-derived basis (e.g. swap-vs-linker
breakeven basis), NO inflation-risk-premium adjustment, and NO
routing through a fitted curve object.  Each endpoint ZCIS rate is
the raw par-rate from instrument_master ×
v_market_data_daily_enriched.  The wire-locked promise is surfaced
via ``current_metrics.methodology_label`` so a downstream operator
cannot misread the output as a cleaner expected-inflation
curvature.
...
call, then align + trim ourselves.  The buffer math matches the
level primitive's own buffer math (``max(z_window, trailing_window)
* buffer_multiplier``) so this primitive does not double-buffer.

Per-trade-date alignment
------------------------
The three endpoint ZCIS series are inner-joined on date so we only
emit rows where ALL THREE endpoints had data — matches the
real_yield_butterfly / breakeven_butterfly outer alignment step.
No ffill across the join: dates where any endpoint has no data are
dropped from the butterfly series, NOT carried forward as
synthetic butterfly points.
...
def _conventions_from_config(config: ToolConfig) -> dict:
    """Pull the methodology kwargs the inflation_swap_butterfly
...
# Fields the same-curve invariant guarantees all three legs share.  A
# mismatch surfaces a controlled error envelope — that would indicate
# an instrument_master inconsistency at the underlying pillars, not a
# methodology choice.
...
def _assert_same_curve_reference_metadata(
    *,
    ...
    """Re-assert that the three endpoint reads share the load-bearing
    ...
    return (
        ...
        f"between {short_tenor} / {belly_tenor} / {long_tenor} legs: "
        + "; ".join(mismatches)
        + ".  The same-curve invariant guarantees these fields are "
        "identical across all three pillars; a mismatch indicates a "
        "real instrument_master inconsistency at the underlying "
        "pillars.  Refer to the inflation_swaps playbook to confirm "
        "the intended convention before requesting this butterfly."
    )
...
def calculate_inflation_swap_butterfly(
    engine: Engine,
    params: InflationSwapButterflyInput,
    config: Optional[ToolConfig] = None,
) -> Dict[str, Any]:
    """Calculate the same-curve ZCIS butterfly (3-point curvature)
    on a single ZCIS curve family.
    ...
    -------
    dict
        Serialised ``InflationSwapButterflyOutput``, or
    ...
    # picking one side.
    # ------------------------------------------------------------------
    short_metrics_inner = inner_results["short"].get("current_metrics", {})
    belly_metrics_inner = inner_results["belly"].get("current_metrics", {})
    long_metrics_inner = inner_results["long"].get("current_metrics", {})
    ref_err = _assert_same_curve_reference_metadata(
        short_metrics=short_metrics_inner,
        belly_metrics=belly_metrics_inner,
    ...
    # ALL THREE endpoints had data — no synthetic butterfly points
    # are produced on dates where any endpoint has no data.
    # ------------------------------------------------------------------
        ...
        if not rows:
            return {
                "error": (
                    ...
                    "the level primitive returned a snapshot but "
                    "no history.  Please report with the "
                    f"curve_family {params.curve_family} and tenors "
                    f"{params.short_tenor} / {params.belly_tenor} / "
                    f"{params.long_tenor}."
                )
            }
        col_name = f"{endpoint_label}_zcis_rate_pct"
    ...
    if wide.empty:
        return {
            "error": (
                f"After aligning the three endpoint ZCIS series for "
                f"{params.curve_family} ({params.short_tenor} → "
                f"{params.belly_tenor} → {params.long_tenor}), no "
                "overlapping dates remain.  Verify all three "
                "pillars have data on this curve."
            )
        }

    # ------------------------------------------------------------------
    # Butterfly formula:
    #   butterfly_bps = (belly_pct - 0.5*(short_pct + long_pct)) * 100
    ...
    # butterfly once at the end of this arithmetic.  Same boundary-
    # rounding discipline as the sibling inflation_swap_curve_spread
    # primitive and the breakeven_butterfly primitive.  Wing spreads
    # use the same boundary-rounding so the snapshot's wing_*_bps
    # fields agree with the desk's manual decomposition.
    # ------------------------------------------------------------------
    short_series = wide["short_zcis_rate_pct"]
    belly_series = wide["belly_zcis_rate_pct"]
    long_series = wide["long_zcis_rate_pct"]
    wide["butterfly_bps"] = (
        (belly_series - 0.5 * (short_series + long_series)) * 100
    ).round(bps_round_decimals)
    wide["wing_short_bps"] = (
        (belly_series - short_series) * 100
    ).round(bps_round_decimals)
    wide["wing_long_bps"] = (
        (long_series - belly_series) * 100
    ).round(bps_round_decimals)
    ...
    # latest aligned trade_date.  Matches inflation_swap_curve_spread
    # / breakeven_butterfly / real_yield_butterfly / OIS rate_level.
    # ------------------------------------------------------------------
    ...
    if display_df.empty:
        return {
            "error": (
                f"No ZCIS butterfly observations within the last "
    ...
    # Trailing high/low/percentile of the ZCIS butterfly (bps scale).
    ...
    metrics = InflationSwapButterflyCurrentMetrics(
        ...
        belly_zcis_rate_pct=safe_float(
            latest.get("belly_zcis_rate_pct"),
            decimals=yield_round_decimals,
        ),
    ...
    output = InflationSwapButterflyOutput(
...
def _series_name_slug(
    *,
    ...
    ``<curve_family>_<short>_<belly>_<long>``.
    """
    return (
...
def _build_canonical_butterfly_series(
    ...
    *,
    ...
    sovereign nominal butterflies, real-yield butterflies, and
    breakeven butterflies when they end up in the same operator
    panel downstream.
    """
    ...
    return TimeSeries(
        series_name=f"{slug}_inflation_swap_butterfly",
        units=TimeSeriesUnits.BPS,
        description=(
            f"Same-curve zero-coupon inflation swap (ZCIS) butterfly "
            f"({curve_family}) "
            f"{short_tenor}/{belly_tenor}/{long_tenor} "
            "((belly_zcis_pct - 0.5*(short_zcis_pct + "
            "long_zcis_pct)) * 100) over the displayed window.  "
            "Curvature of the ZCIS rate curve in raw inflation-"
            "swap-rate space — NO subtraction of a model-derived "
            "basis, NO inflation-risk-premium adjustment, NO routing "
            "through a fitted curve object.  Reference: "
            ...
            "curve butterflies are interpretable under their own "
            "curve's convention only.  Quoted in BPS (matching the "
            "inflation_swaps domain's BPS convention for curve-shape "
            "views established via inflation_swap_curve_spread)."
        ),
        rows=rows,
    )
...
def _build_canonical_zscore_series(
    ...
    *,
    ...
    return TimeSeries(
        series_name=f"{slug}_inflation_swap_butterfly_zscore",
        units=TimeSeriesUnits.Z_SCORE,
        description=(
            f"Rolling z-score of the {curve_family} ZCIS butterfly "
            f"(in bps) over {short_tenor}/{belly_tenor}/{long_tenor} "
