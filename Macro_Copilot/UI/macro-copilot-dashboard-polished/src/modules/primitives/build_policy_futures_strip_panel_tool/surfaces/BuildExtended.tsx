...
// ``build_policy_futures_strip_panel_tool``.  PANEL-CONTRACT shape.
// ----------------------------------------------------------------------------
// Per rendering_density.md §2.1 + §5 every primitive claiming
// ``custom_build_surface`` ships an extended view; this is the policy-
// futures strip panel's full canvas.  Mounted by VirtualPrimitiveCanvas
// for single-tool queries OR by the click-to-expand modal from the
// compact card.
//
// PANEL shape (settled, sovereign-sibling precedent): the wire carries
// the panel's METADATA CONTRACT (dims / '<CURVE_FAMILY>|<STRIP_POSITION>'
// column keys / date range / units / methodology card) — NOT the cell
// data, which stays a workflow-side Panel artifact (the MCP layer and
// the detail route both drop it).  So this canvas is a CONTRACT card:
// controls → hero KPI strip → column roster → verbatim methodology-card
// caveats (inverse pricing / rolling generics / cross-region calendar /
// per-family RFR-IBOR + Buba mix) → lineage.  No chart — there is no
...
import {
  ...
  fmtInt,
  heroKPIs,
  usePolicyFuturesStripPanel,
  type StripPanelRosterRow,
} from './policyFuturesStripPanelShared';
...
const BuildExtended: React.FC<BuildExtendedProps> = ({
  ...
  // Families / positions default BLANK on purpose — omitted scope means
  // the full SOFR/SONIA/Euribor × 1..8 universe (no 422 on a blank
  // Library invocation; see policyFuturesStripPanelShared.ts).
  const fallbackWindow = defaultWindow();
  const curveFamiliesCsv =
    params.curve_families || DEFAULT_CURVE_FAMILIES_CSV;
  const stripPositionsCsv =
    params.strip_positions || DEFAULT_STRIP_POSITIONS_CSV;
  ...
  const { data, isLoading, errorMessage } = usePolicyFuturesStripPanel({
    curveFamiliesCsv,
    stripPositionsCsv,
    startDate,
    endDate,
  ...
  const handleReset = () => {
    pushParams({
      ...
      strip_positions: DEFAULT_STRIP_POSITIONS_CSV,
  ...
  const controls: ReadonlyArray<ControlDescriptor> = [
    {
      name: 'curve_families',
      label: 'Families (CSV, blank → all 3)',
      kind: 'text',
      value: curveFamiliesCsv,
    },
    {
      name: 'strip_positions',
      label: 'Strip positions (CSV 1–8, blank → all)',
      kind: 'text',
      value: stripPositionsCsv,
    },
    {
      name: 'start_date',
    ...
    {
      name: 'field_name',
      label: 'field_name (blank → PX_LAST)',
      kind: 'text',
      value: fieldName,
      advanced: true,
    },
  ...
  return (
    <div
      ...
      data-testid="policy-futures-strip-panel-extended"
    >
      {/* ---------- Title section ---------- */}
      <section className="grid grid-cols-1 gap-4 border-b border-line-subtle px-6 pt-5 pb-5 lg:grid-cols-[1fr_auto]">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[11px] uppercase tracking-wide text-fg-muted">
            ...
            <span className="font-semibold">POLICY-FUTURES STRIP PANEL</span>
          ...
          <div className="flex items-baseline gap-3">
            <h1 className="text-[24px] font-semibold leading-tight text-fg-primary">
              Policy-Futures Strip Panel
            </h1>
            <span className="text-[13px] text-fg-secondary">
              (STIR implied-rate backtest substrate)
            </span>
          </div>
...
function RosterTable({
  ...
  rows: ReadonlyArray<StripPanelRosterRow>;
  isLoading?: boolean;
  errorMessage?: string;
}) {
  return (
    <div className="card overflow-hidden">
      <div className="border-b border-line-subtle px-4 py-3">
        <span className="kicker text-fg-muted">
          Column roster — '&lt;CURVE_FAMILY&gt;|&lt;STRIP_POSITION&gt;' keys
        </span>
      </div>
            ...
            {errorMessage ? (
              ...
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-fg-secondary">
                  No columns resolved — check the family / position scope.
                </td>
              </tr>
            ) : (
              rows.map((row, i) => (
