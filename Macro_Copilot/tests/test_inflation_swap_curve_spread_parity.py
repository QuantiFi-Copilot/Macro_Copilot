"""
test_inflation_swap_curve_spread_parity.py — Snapshot parity tests
for calculate_inflation_swap_curve_spread
==================================================================

PR15 backfill — modelled on
``tests/test_inflation_swap_rate_level_parity.py`` (the inner level
primitive, stage 1) and the stage-2 breakeven compose primitives.
...
same-curve reference-metadata equality guard is data-derived from
the inner primitive's ``current_metrics``, which comes from the
captured 8-column rows — no separate mock needed.

Regenerating fixtures — see
``tests/fixtures/inflation_swap_curve_spread_v1/README.md``.
"""
...
FIXTURES_DIR = (
    Path(__file__).parent / "fixtures" / "inflation_swap_curve_spread_v1"
)
FLOAT_ABS_TOL = 1e-9


class _FrozenDateForZcisLevel(date):
    """Frozen-date subclass patched into
    ``inflation_swap_rate_level.compute.date`` so the inner level
    primitive's one ``date.today()`` callsite becomes deterministic
    across both inner endpoint calls this compose primitive makes
    (the compose primitive does not import ``date`` itself)."""
...
def _build_fetch_side_effect(raw_rows: dict[str, list[dict]]):
    """Side effect for ``fetch_zcis_single_pillar``.  Reconstructs
    the 8-column long-format DataFrame the live fetcher returns —
    ``trade_date`` (datetime.date), ``field_value`` (float), and the
    six object-dtype reference-metadata columns.
    """
...
def test_inflation_swap_curve_spread_parity(fixture_path: Path) -> None:
    ...
    params = InflationSwapCurveSpreadInput(**fx["input"]["params"])
    ...
    with patch(
        ...
        actual = calculate_inflation_swap_curve_spread(
