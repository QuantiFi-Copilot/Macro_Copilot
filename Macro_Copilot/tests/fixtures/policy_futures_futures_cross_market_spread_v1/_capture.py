...
_capture.py — Live-DB parity-fixture capture for
              calculate_futures_cross_market_spread (policy_futures
              domain)
========================================================================

Records the *real* production output of
``calculate_futures_cross_market_spread`` for one representative
matched-strip cross-market snapshot against the live TimescaleDB.
...
across markets — US vs UK trading calendars differ for the SOFR_FUT
vs SONIA_FUT pair, US vs Euro for SOFR_FUT vs EUR_SHORT_RATE_FUT —
SCD2 metadata rotation across each leg's rolling chain, ticks at
0.0025 vs 0.005 across the SFR strip and 0.005 across SFI / ER)
that synthetic generators don't reproduce.
...
2. For the captured case, calls ``fetch_cross_market_strip`` to
   record the exact long-format rows the tool's price fetcher
   returns for the two legs AND ``fetch_strip_position_reference``
   for each leg to record the per-strip metadata as of the pinned
   anchor date.
3. Calls ``calculate_futures_cross_market_spread`` (against the
...
``test_policy_futures_futures_cross_market_spread_parity.py`` is
...
from rates_agent.policy_futures.tools.futures_cross_market_spread.compute import (  # noqa: E402
    CONFIG_PATH as _CROSS_MARKET_SPREAD_CONFIG_PATH,
)
...
_CASES: list[dict] = [
    {
        "fixture_name": "sofr_sonia_strip1",
        "params": {
...
def _row_dict(
    trade_date: Any, curve_family: str, field_value: float,
) -> dict:
...
def _normalise_reference(reference: dict) -> dict:
    """Coerce DB-driver types (Decimal, date) into JSON-safe
    scalars."""
...
def _capture_one(case: dict, engine, captured_at: str, db_name: str) -> dict:
    ...
    params = FuturesCrossMarketSpreadInput(**case["params"])

    cfg = load_tool_config(_CROSS_MARKET_SPREAD_CONFIG_PATH)
    ...
    raw_price_df = fetch_cross_market_strip(
        engine=engine,
    ...
    if raw_price_df.empty:
        raise RuntimeError(
            ...
            f"fetch_cross_market_strip returned 0 {price_field} rows."
        )
    ...
    raw_price_rows = [
        _row_dict(
            r["trade_date"], r["curve_family"], r["field_value"],
        )
        for _, r in raw_price_df.iterrows()
    ]
    raw_price_rows.sort(
        key=lambda r: (r["trade_date"], r["curve_family"]),
    )
    ...
    expected_output = calculate_futures_cross_market_spread(
    ...
    fixture = {
        ...
        "tool_function": "calculate_futures_cross_market_spread",
        "capture": {
        ...
        "input": {
            ...
            "reference_a": _normalise_reference(reference_a),
            "reference_b": _normalise_reference(reference_b),
        },
...
def main() -> None:
    ...
    for case in _CASES:
        ...
        print(
            ...
            f"spread_pct={cm.get('spread_value_pct', '?')}"
        )
        successes += 1
