...
// the chip strip is a wrong-family-list tripwire, not a roster.
...
const BuildCompact: React.FC<BuildCompactProps> = ({
  ...
  const columnKeys = data?.vendor_tickers ?? [];
  ...
  return (
    <article
      ...
      style={{ minHeight }}
      data-testid="linker-panel-compact"
    >
      {/* ---------- Header ---------- */}
      <header className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[13px] font-semibold text-fg-primary">
              <span className="truncate">INFLATION-LINKER PANEL</span>
            ...
            <div className="mt-0.5 flex items-center gap-1.5 text-[12px] text-fg-secondary">
              <span className="truncate">
                Real-yield panel metadata — cells stay workflow-side
              </span>
              {callMeta && (
        ...
        {onExpand && (
          <button
            type="button"
            onClick={onExpand}
            aria-label="Open extended view of the inflation-linker panel contract"
