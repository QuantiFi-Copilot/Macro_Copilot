"""
test_futures_price_level_parity.py — Snapshot parity tests for the
                                       bond-futures price-level monitor
=====================================================================

Locks in the *real production* output of
``calculate_futures_price_level`` for one or more representative
...
PR15 binds new primitives from day-one — ``futures_price_level`` is a
...
``tests/fixtures/futures_price_level_v1/_capture.py``; the test itself
runs fully offline by replaying captured ``raw_rows`` through a
mocked fetcher.

How it works
------------
For each fixture in ``tests/fixtures/futures_price_level_v1/``:
...
3. Patch
   ...
   to return that DataFrame, and patch
   ...
   ``input.frozen_today``, so the two ``date.today()`` callsites inside
   the tool become deterministic. The patches target ``compute.py``'s
   namespace specifically because the package ``__init__.py`` only re-
   exports public symbols and does NOT propagate ``compute.py``'s
   imports — see the package init's "Note for tests" section.
4. Build a ``FuturesPriceLevelInput`` from the recorded params and call
   ``calculate_futures_price_level(engine=None, params=...)``.
...
(z-score to 4 dp, price quantities to 6 dp), so on the same inputs the
result *should* be bit-identical between runs. We still allow a 1e-9
...
``tests/fixtures/futures_price_level_v1/README.md``.
"""
...
FIXTURES_DIR = Path(__file__).parent / "fixtures" / "futures_price_level_v1"
...
class _FrozenDateForFuturesPriceLevel(date):
...
def _discover_fixtures() -> list[Path]:
    ...
    and are skipped — only ``_capture.py`` is allowed to write into this
...
def test_futures_price_level_parity(fixture_path: Path) -> None:
    ...
    #    fresh recompute. If raw_rows were edited by hand without re-
    #    running the capture script, the hash diverges and we abort
    ...
    # 3. Reconstruct the reference dict the metadata fetcher would
    #    return. ``expiry_date`` is recorded as a YYYY-MM-DD string in
    #    the fixture; compute.py accepts string OR date. We pass the
    #    string through (matches what the live fetcher returns).
    reference = fx["input"]["reference"]

    # 4. Build the input.
    params = FuturesPriceLevelInput(**fx["input"]["params"])

    # 5. Freeze the date.
    _FrozenDateForFuturesPriceLevel._frozen_value = date.fromisoformat(
        fx["input"]["frozen_today"]
    )

    # 6. Patch and run. Patches target the COMPUTE module specifically.
    target = "rates_agent.bond_futures.tools.futures_price_level.compute"
    with patch(f"{target}.fetch_rolling_generic_series", return_value=raw_df), \
         patch(f"{target}.fetch_rolling_generic_reference", return_value=reference), \
         patch(f"{target}.date", _FrozenDateForFuturesPriceLevel):
