#!/usr/bin/env python3
"""
test_scan_inflation_linkers_extremes_sql_validation.py — linker
                                                          universe-
                                                          scan
                                                          validator

Independently reproduces the linker universe-extremes scan in SQL:
for each ``(curve_family, tenor)`` stem in the linker universe,
compute the 252-day rolling z-score of the cleaned real-yield
LEVEL series on the enriched view, pick the top-N stems by |z|,
and assert the Python tool's ranking matches the SQL ranking row-
for-row.
...
have IGNORE NULLS on LAST_VALUE.  Workaround: since linker daily
series are almost entirely gap-free on each curve_family's own
...
# Linker V1 universe — matches the YAML's
# ``inflation_linker_curve_families`` whitelist.
LINKER_CURVE_FAMILIES: Tuple[str, ...] = (
...
REGRESSION_CASES: List[Case] = [
    ...
    (5, 0.0, PINNED_ANCHOR_DATE, LINKER_CURVE_FAMILIES),
    # USD_TIPS + GBP_LINKER, default-style threshold.
    (3, 1.0, PINNED_ANCHOR_DATE, ("USD_TIPS", "GBP_LINKER")),
    # USD_TIPS only, high top_n.
    (10, 0.0, PINNED_ANCHOR_DATE, ("USD_TIPS",)),
    # Full universe, data-max anchor.
    (5, 0.0, None, LINKER_CURVE_FAMILIES),
]
...
TOLERANCE_BY_FIELD = {
    "z_score_real_yield": 6e-3,
    "real_yield_pct": 1e-3,  # 4-decimal rounding; harmless float diff
    "daily_change_bps": 0.01,
    "monthly_change_bps": 0.01,
}
...
def sql_per_stem_z_scores(
    ...
    *,
    ...
    families, compute the rolling 252d z-score of the real-yield
    ...
    sql = text(
        ...
        raw_yields AS (
            SELECT
                ...
                tenor,
                trade_date,
                field_value::double precision AS real_yield
        ...
        dedup AS (
            ...
            -- linker rows; this is defensive.
            SELECT DISTINCT ON (curve_family, tenor, trade_date)
                curve_family, tenor, trade_date, real_yield
            FROM raw_yields
            ORDER BY curve_family, tenor, trade_date, real_yield
        ),
        scored AS (
            SELECT
                curve_family,
                ...
                CASE
                    WHEN COUNT(*) OVER zw >= 60
                     AND STDDEV_SAMP(real_yield) OVER zw IS NOT NULL
                     AND STDDEV_SAMP(real_yield) OVER zw <> 0
                    THEN ROUND(
                        ((real_yield - AVG(real_yield) OVER zw)
                         / NULLIF(STDDEV_SAMP(real_yield) OVER zw, 0))
                            ::numeric,
                        4
                    )::double precision
                    ELSE NULL
                ...
                -- compute layer.
                ROUND(
                    ((real_yield - LAG(real_yield, 1) OVER (
                        PARTITION BY curve_family, tenor
                        ORDER BY trade_date
                    )) * 100.0)::numeric, 2
                )::double precision AS daily_change_bps,
                ROUND(
                    ((real_yield - LAG(real_yield, 21) OVER (
                        PARTITION BY curve_family, tenor
                        ORDER BY trade_date
                    )) * 100.0)::numeric, 2
                )::double precision AS monthly_change_bps
            FROM dedup
            WINDOW zw AS (
                PARTITION BY curve_family, tenor
                ORDER BY trade_date
        ...
        per_stem_latest AS (
            SELECT DISTINCT ON (curve_family, tenor)
                ...
                ROUND(real_yield::numeric, 4)::double precision
                    AS real_yield_pct,
                daily_change_bps,
                monthly_change_bps,
                z_real_yield
...
def sql_ranked_rows(
    ...
    *,
    ...
    return [
        {**row, "z_score_real_yield": z, "rank": idx}
...
def compare_results(
    ...
    for rank_idx, (t_row, s_row) in enumerate(
        ...
        if t_row["tenor"] != s_row["tenor"]:
            ...
            continue
        for field in (
            "z_score_real_yield", "real_yield_pct",
            "daily_change_bps", "monthly_change_bps",
        ):
            sql_field = (
                "z_real_yield" if field == "z_score_real_yield"
                else field
            )
            t_v = t_row.get(field)
...
def run_case(
    ...
    *,
    ...
    tool_result = calculate_scan_inflation_linkers_extremes(
        engine=engine,
        params=ScanInflationLinkersExtremesInput(
            curve_families=(
                list(curve_families)
                if curve_families != LINKER_CURVE_FAMILIES
                else None
            ),
            top_n=top_n,
...
# BEYOND the linker universe's last observed ``trade_date``, the
...
def run_future_anchor_guard_case(engine, field_name: str) -> List[str]:
    ...
    1. Pull the linker universe's current MAX(trade_date) in SQL.
    ...
    with engine.connect() as conn:
        max_row = conn.execute(
            ...
            {
                "curve_families": list(LINKER_CURVE_FAMILIES),
                "instrument_type": INSTRUMENT_TYPE,
                "field_name": field_name,
            },
        ).first()
    if max_row is None or max_row[0] is None:
        return [
            ...
            "MAX(trade_date) — linker universe appears empty; cannot "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the linker universe-extremes scan against "
            "direct SQL."
        ),
    )
    parser.add_argument(
    ...
    print("=" * 80)
    print("LINKER UNIVERSE-EXTREMES SCAN — SQL VALIDATION")
    print("=" * 80)
    ...
    def _case_label(c: Case) -> str:
        ...
        return (
            ...
            f"curves={'(all)' if c[3] == LINKER_CURVE_FAMILIES else list(c[3])}"
        )
