...
| v6.1 | 2026-05-23 | C4 Phase A.4 amendment. **Closes the chain-cutoff blocker** identified in v6's pending-work note. Operator one-line probe (`bds("TY1 Comdty", "FUT_CHAIN", INCLUDE_EXPIRED_CONTRACTS="Y", DT_OVERRIDE_START="1995-01-01")`) returned `0` -- Bloomberg does NOT honor `DT_OVERRIDE_START` on `FUT_CHAIN`. Per Codex's B-first-A-fallback plan: falling back to operator-controlled in-extractor capping. v6.1 adds: (a) optional `chain_max_length: dict[vendor_ticker, positive_int]` field on the `deliverables:` section. The validator (`_resolve_deliverables_section`) accepts an empty-or-absent mapping as "no caps", rejects non-dict / empty-dict / blank-key / non-string-key / non-positive-int-value / non-int-value / bool-value (the last because `True isinstance int` in Python and a YAML `chain_max_length: {"TY1 Comdty": true}` typo would silently behave as `cap=1` without the explicit check). (b) New pure helper `_apply_chain_max_length(contracts, max_length) -> contracts'` in both extractors (SYNC INVARIANT). `None` cap returns the chain unchanged; cap >= len returns the chain unchanged; cap < len returns `contracts[-cap:]` (the newest N entries; Bloomberg's `FUT_CHAIN` returns chronological oldest-first order per the operator probe). Defence-in-depth `ValueError` on non-positive/non-int cap at call time (the validator catches it earlier). (c) `run_deliverables_extraction` in both extractors looks up the per-generic cap from `section["chain_max_length"]` after the chain fetch and applies `_apply_chain_max_length`. A capped chain logs `[INFO] chain_max_length=N applied for <ticker>: trimmed M -> N contracts (newest kept)`. The v3 strict contract-level coverage gate then operates only on the trimmed chain -- the oldest contracts (pre-Bloomberg-basket-cutoff) are excluded from the load entirely, never counted as misses. (d) Per-generic caps set in `bond_futures.yml` per the operator probe's per-generic data cutoffs, with ~5y safety margin past each: `TU1=108` (oldest kept ~1999.5), `FV1=110` (~2000.75), `TY1=128` (~1994.75), `US1=140` (~1991.25). UXY1 and WN1 omitted (full Bloomberg coverage, no cap needed). The operator can tune values up (more history) or down (more safety) in subsequent runs based on production-extractor results. **25 new tests**: `TestApplyChainMaxLength` (9 cases -- None/larger/equal/smaller cap, real US1 simulation, fail-closed paths including the bool-as-int edge case), `TestResolveDeliverablesSectionChainMaxLengthV6_1` (12 cases -- absent/well-formed/empty/non-dict/blank-key/non-string-key/negative/zero/non-int/bool/partial-coverage), `TestChainMaxLengthHistoricalParity` (4 cases SYNC INVARIANT). Total C3+C4 deliverables tests: **177 pass**. Schema, natural key, ingester route, ingester parser, v3 coverage gate, v4 validator, v5 parse-validation, v6 CUSIP canonicaliser unchanged. |
