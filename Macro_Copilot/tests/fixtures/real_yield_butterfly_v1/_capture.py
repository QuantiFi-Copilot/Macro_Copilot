...
``calculate_real_yield_butterfly`` for three representative linker
curves (USD_TIPS 5s10s30s, GBP_LINKER 5s10s30s, USD_TIPS 5s20s30s).
...
---------------------------------
The primitive composes three calls to ``get_real_yield_level`` and
runs one identity-guard DB read in its OWN compute module.  The
capture records:

  - ``country_currency`` — ONE row per scenario from
    ``_fetch_curve_family_country_currency`` for
    ``(curve_family, instrument_type='inflation_linker')``.
  - ``raw_rows`` — keyed by
    ``"<curve_family>__<tenor>__<field_name>__<instrument_type>"``,
    THREE entries per scenario (short, belly, long).
  - ``expected_output`` — full primitive output from the live-DB
    run.

The parity test patches THREE seams:

  - ``real_yield_butterfly.compute._fetch_curve_family_country_currency``
...
from rates_agent.inflation_indexed_bonds.tools.real_yield_butterfly.compute import (  # noqa: E402
    CONFIG_PATH as _RYB_CONFIG_PATH,
...
_CASES: list[dict] = [
    {
        "fixture_name": "usd_tips_5s10s30s_365d",
        "params": {
    ...
    {
        "fixture_name": "gbp_linker_5s10s30s_365d",
        "params": {
    ...
    {
        "fixture_name": "usd_tips_5s20s30s_365d",
        "params": {
            "curve_family": "USD_TIPS",
            "short_tenor": "5Y",
            "belly_tenor": "20Y",
...
def _capture_one(case: dict, engine, captured_at: str, db_name: str) -> dict:
    params = RealYieldButterflyInput(**case["params"])
    frozen_today = date.today()

    cfg = load_tool_config(_RYB_CONFIG_PATH)
    ...
    # Same nested-buffer math as real_yield_curve_spread.
    ...
    expected_output = calculate_real_yield_butterfly(
    ...
    return {
        ...
        "tool_function": "calculate_real_yield_butterfly",
        "capture": {
...
def main() -> None:
    ...
    for case in _CASES:
        ...
        print(
            ...
            f"butterfly={m['current_butterfly_pct']:+.4f}pct  "
