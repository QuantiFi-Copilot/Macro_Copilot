"""
test_real_yield_butterfly_parity.py — Snapshot parity tests for
...
curves.  PR15 backfill — modelled on
``tests/test_real_yield_curve_spread_parity.py``; the only shape
difference is THREE inner ``fetch_single_tenor`` calls per scenario
(short / belly / long) instead of two.
...
---------------------
See ``tests/fixtures/real_yield_butterfly_v1/README.md``.
"""
...
FIXTURES_DIR = (
    Path(__file__).parent / "fixtures" / "real_yield_butterfly_v1"
)
FLOAT_ABS_TOL = 1e-9
...
class _FrozenDateForRealYieldLevel(date):
    ...
    ``real_yield_level.compute.date`` so the inner level primitive's
    one ``date.today()`` callsite becomes deterministic."""
...
def test_real_yield_butterfly_parity(fixture_path: Path) -> None:
    ...
    params = RealYieldButterflyInput(**fx["input"]["params"])
    ...
    with patch(
        ...
        actual = calculate_real_yield_butterfly(
