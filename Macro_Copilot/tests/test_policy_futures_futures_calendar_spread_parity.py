"""
test_policy_futures_futures_calendar_spread_parity.py — Snapshot
                                                         parity tests for
                                                         the policy-
                                                         futures calendar-
                                                         spread monitor
...
``calculate_futures_calendar_spread`` (policy_futures domain) for
one or more representative pair invocations (default:
``SOFR_FUT`` strip_position_short=1, strip_position_long=2 anchored
at 2026-04-08). Any commit that changes the math — directly or
through a primitive in ``shared/analytics/`` or via a YAML-convention
edit — must keep this test green or come with a deliberate, reviewed
...
``tests/fixtures/policy_futures_futures_calendar_spread_v1/_capture.py``;
...
------------
For each fixture in
``tests/fixtures/policy_futures_futures_calendar_spread_v1/``:

1. Load the JSON. Verify the captured ``raw_rows_sha256`` matches a
   freshly-computed hash of ``input.raw_price_rows`` +
   ``input.reference_short`` + ``input.reference_long`` — guards
   against fixture tampering.
2. Reconstruct the long-format DataFrame the tool's DB fetcher
   would return for PX_LAST across both legs.
3. Patch the compute module's fetchers + ``date`` reference with a
   subclass whose ``today()`` returns ``input.frozen_today``.
4. Build a ``FuturesCalendarSpreadInput`` from the recorded params
   and call ``calculate_futures_calendar_spread``.
...
----------------------------------------
``calculate_futures_calendar_spread`` already rounds every numeric
output, so on the same inputs the result *should* be bit-identical
between runs. We still allow a 1e-9 absolute tolerance to absorb
harmless float-formatting differences across pandas / numpy
versions — anything bigger than that is real math drift and the
test fails.

Regenerating fixtures
---------------------
Only after a deliberate methodology change. See
``tests/fixtures/policy_futures_futures_calendar_spread_v1/README.md``.
"""
...
FIXTURES_DIR = (
    ...
    / "fixtures"
    / "policy_futures_futures_calendar_spread_v1"
)
...
class _FrozenDateForMonitor(date):
    ...
    Patched in place of
    ``rates_agent.policy_futures.tools.futures_calendar_spread.compute.date``
...
def test_policy_futures_futures_calendar_spread_parity(
    fixture_path: Path,
) -> None:
    """Run the captured inputs through ``calculate_futures_calendar_spread``
    ...
    reference_short = fx["input"]["reference_short"]
    reference_long = fx["input"]["reference_long"]
    ...
    # 2. Reconstruct the long-format DataFrame the strip-group fetcher
    #    returns.
    ...
    params = FuturesCalendarSpreadInput(**params_dict)
    ...
    with patch(
        ...
        actual = calculate_futures_calendar_spread(
...
def test_policy_futures_futures_calendar_spread_parity_tamper_detected() -> None:
    ...
    tampered_hash = _hash_payload(
        tampered, fx["input"]["reference_short"],
        fx["input"]["reference_long"],
    )
