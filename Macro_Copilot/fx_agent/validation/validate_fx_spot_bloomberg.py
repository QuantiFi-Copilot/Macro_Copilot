...
OUTPUT_TXT = "fx_spot_validation_report.txt"
OUTPUT_TICKER_CSV = "fx_spot_validation_ticker_summary.csv"
OUTPUT_FIELD_CSV = "fx_spot_validation_field_level.csv"
OUTPUT_FAMILY_CSV = "fx_spot_validation_family_summary.csv"
...
def normalize_bdh_output(df: Any, requested_field: str) -> pd.DataFrame:
    ...
    return out[[out.columns[0], value_col]].rename(columns={out.columns[0]: "date", value_col: "value"})
...
def main() -> None:
    ...
    logger.log("=" * 140)
    logger.log("FX SPOT BLOOMBERG TICKER AND FIELD VALIDATION")
    logger.log("=" * 140)
    logger.log(f"Total pairs tested     : {len(FX_SPOT_UNIVERSE)}")
    ...
    for pair, cfg in FX_SPOT_UNIVERSE.items():
        ticker = cfg["ticker"]

        logger.log("-" * 140)
        logger.log(
            f"{pair} | ticker={ticker} | fx_family={cfg['fx_family']} | "
            f"base={cfg['base_ccy']} | quote={cfg['quote_ccy']}"
        )

        batch_ref_values, batch_ref_err = safe_bdp_batch(ticker, REFERENCE_FIELDS)
        logger.log(f"Batch BDP request: {'FAIL' if batch_ref_err else 'OK'}")
        if batch_ref_err:
            logger.log(f"Batch BDP error  : {batch_ref_err}")
        ...
        for field in REFERENCE_FIELDS:
            ...
            logger.log(
                f"{field:<16} | pass={'YES' if passed else 'NO ':<3} | "
                f"value={repr(value_for_eval)} | note={note}"
            )
            if single_err:
        ...
        logger.log(f"Batch BDH request: {'FAIL' if batch_hist_err else 'OK'}")
        if batch_hist_err:
            logger.log(f"Batch BDH error  : {batch_hist_err}")
        ...
        for field in TARGET_FIELDS:
            ...
            passed = summary["non_null_points"] >= MIN_HISTORY_POINTS
            ...
            field_rows.append({
                ...
                "pass": passed,
                "note": note,
                "history_points": summary["non_null_points"],
            ...
            logger.log(
                f"{field:<16} | pass={'YES' if passed else 'NO ':<3} | "
                f"points={summary['non_null_points']:<4} | "
                f"first_date={summary['first_date']} | last_date={summary['last_date']} | "
                f"first_value={summary['first_value']} | last_value={summary['last_value']} | "
        ...
        logger.log(
            f"reference_complete={'YES' if reference_complete else 'NO'} | "
            f"history_complete={'YES' if history_complete else 'NO'} | "
            f"ticker_ready_for_playbook={'YES' if ticker_ready else 'NO'}"
        )
        logger.log()

    family_rows.append({
        "fx_family": "G10_SPOT",
        "pairs_tested": len(FX_SPOT_UNIVERSE),
    ...
    total_pairs = len(ticker_df)
    total_ready = int(ticker_df["ticker_ready_for_playbook"].sum()) if not ticker_df.empty else 0
    total_failed_field_checks = int((~field_df["pass"]).sum()) if not field_df.empty else 0

    logger.log("=" * 140)
    logger.log("FINAL SUMMARY")
    logger.log("=" * 140)
    logger.log(f"Pair pass count         : {total_ready} / {total_pairs}")
    logger.log(f"Failed field checks     : {total_failed_field_checks}")
    ...
    logger.log(f"Saved family summary to : {Path(OUTPUT_FAMILY_CSV).resolve()}")
    logger.save()

    print("=" * 100)
    print("FX SPOT VALIDATION COMPLETE")
    print("=" * 100)
...
if __name__ == "__main__":
    try:
        ...
        Path(OUTPUT_TXT).write_text(
            "Fatal error while running FX spot validation.\n\n" + traceback.format_exc(),
            encoding="utf-8",
        )
        raise