...
2. Its inputSchema is a well-formed JSON-Schema object with nested
   ``$defs`` for SeriesSpec.
3. The schema correctly types target_spec as a SeriesSpec ref and
   regressor_specs as an array of SeriesSpec refs.
4. The schema's required-fields set matches the tool's required
   parameters.

The tests use FastMCP's introspection API
(``mcp.server.fastmcp.FastMCP.list_tools``) — the same API the MCP
client uses when negotiating tools with a server.
"""
