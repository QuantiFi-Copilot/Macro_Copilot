...
// primitive ships a compact view; this one is a TOP-N RANKED TABLE (NOT a
// sparkline) per the catalog guardrail.  Mounted as a node body inside
// multi-tool query DAG visualisations.
//
// Design reference: ./mockups/Compact.png (committed alongside this module).
//
// Why a custom inline card vs the shared BuildCompactShell:
//   The shared shell is LEVEL-shape oriented (3 KPI cells + sparkline +
//   methodology footer).  The scanner's canonical "first three things a PM
//   reads off it" are RANKED ROWS, not KPIs.  Per rendering_density.md
//   §2.2 the contract is semantic (identity / headline data / methodology /
//   expand / tone cues) — not file-identical to the level-shape compact
//   card.  This component honours the same semantic contract with a
...
// up to 4 ranked rows + a "MOST EXTREME" highlight row.  Anything beyond is
...
const BuildCompact: React.FC<BuildCompactProps> = ({
  ...
  // Resolve params — the compact card uses the YAML-default top_n /
  // min_abs_z_score (None ⇒ backend fall-through) unless the caller threads
  // explicit overrides through the DAG node params.
  ...
  return (
    <article
      ...
      style={{ minHeight }}
      data-testid="zcis-scanner-compact"
    >
      {/* ---------- Header ---------- */}
      <header className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[13px] font-semibold text-fg-primary">
              <span className="truncate">ZCIS EXTREMES</span>
        ...
        {onExpand && (
          <button
            type="button"
            onClick={onExpand}
            aria-label="Open extended view of ZCIS extremes"
      ...
      {errorMessage ? (
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-2">
          ...
          <div className="grid grid-cols-[24px_1fr_auto_auto] items-center gap-3 px-1 text-[10.5px] uppercase tracking-wide text-fg-muted">
            ...
            <span className="text-right">Current Rate (mid)</span>
          ...
          {most && (
            <div className="mt-auto flex items-center justify-between gap-2 rounded-md border border-line-subtle bg-surface-overlay px-2.5 py-2">
              ...
              <div className="flex items-center gap-2 text-[12px]">
                ...
                <span
                  className={`font-mono tabular-nums ${toneTextClass(zScoreTone(most.z_score_zcis_rate))}`}
                >
                  ({formatZScore(most.z_score_zcis_rate)})
                </span>
              </div>
            </div>
          )}
      ...
      <footer className="flex items-center justify-between gap-3 text-[11px]">
        ...
        >
          ...
          <span className="truncate">{ZCIS_SCANNER_COMPACT_CAVEAT}</span>
        </div>
        <div className="flex shrink-0 items-center gap-3 text-fg-muted">
          ...
          {summary.asOfDate && <span>As of {summary.asOfDate}</span>}
