...
// ============================================================================
// src/modules/primitives/calculate_ois_forward_rate_tool/__tests__/module.spec.ts
// ----------------------------------------------------------------------------
...
const FOLDER = 'calculate_ois_forward_rate_tool';
...
check('default curve_family is a recognised OIS curve', () => {
  ...
  if (typeof cf !== 'string' || !RECOGNISED.has(cf)) {
    throw new Error(
      `defaultParams.curve_family must be a recognised OIS family; got ${JSON.stringify(cf)}.`,
    );
  }
});
...
check('default forward_pair has matching start_tenor + end_tenor', () => {
  ...
  // together; the schema layer rejects partial/both modes.  defaultParams
