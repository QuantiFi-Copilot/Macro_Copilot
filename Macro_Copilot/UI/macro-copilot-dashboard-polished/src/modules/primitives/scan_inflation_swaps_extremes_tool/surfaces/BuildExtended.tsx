...
// ``scan_inflation_swaps_extremes_tool``.  SCANNER shape.
// ----------------------------------------------------------------------------
// Per docs_revamped/03_standards/rendering_density.md §2.1 + §5 every new
// primitive ships an extended view; this is the universe-scan canvas.
// Mounted by VirtualPrimitiveCanvas when this tool is the sole focus of a
// single-tool query OR by the click-to-expand modal infrastructure when
// invoked from the compact card in a multi-tool DAG.
//
// Design reference: ./mockups/Extended.png.
//
// Layout — the scanner's headline data is a RANKED LIST, not a time series.
// The standard BuildExtendedShell's chart-centric layout doesn't fit; this
// component owns its own scanner-shaped layout (identity row → top-right
// summary cards → controls strip → distribution histogram → ranked detail
// table → methodology card → lineage footer).  It reuses the shared
// `ControlsStrip` / `MethodologyCard` / `LineageFooter` / `FreshnessPill`
// elements so the chrome matches the rest of the Build catalogue.
// ============================================================================
...
import type { ScanInflationSwapsExtremesResultRow } from '@/types/rates';

// ---------------------------------------------------------------------------
// Defaults — kept here so the controls strip and the fetch hook share one
// source of truth.  The backend resolves None ⇒ YAML default; the extended
// view surfaces the YAML defaults as the displayed value but passes the
// number through to the hook so user overrides round-trip via URL state.
// ---------------------------------------------------------------------------
...
const CURVE_FAMILY_OPTIONS: ReadonlyArray<{ value: string; label: string }> = [
  { value: ALL_CURVE_FAMILIES_VALUE, label: 'All ZCIS (USD / EUR / GBP)' },
  ...ZCIS_SCANNER_CURVE_OPTIONS,
];
...
const BuildExtended: React.FC<BuildExtendedProps> = ({
  ...
  // Resolve effective params — fold defaults for missing keys so the
  // displayed controls round-trip cleanly with URL state.
  ...
  const { data, isLoading, errorMessage } = useZcisScanner({
    curveFamilies,
    topN: Number(topNStr),
  ...
  const highCount = rows.filter((r) => (r.z_score_zcis_rate ?? 0) > 0).length;
  const lowCount = rows.filter((r) => (r.z_score_zcis_rate ?? 0) < 0).length;
  ...
  return (
    <div
      ...
      data-testid="zcis-scanner-extended"
    >
      {/* ---------- Title section ---------- */}
      <section className="grid grid-cols-1 gap-4 border-b border-line-subtle px-6 pt-5 pb-5 lg:grid-cols-[1fr_auto]">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[11px] uppercase tracking-wide text-fg-muted">
            ...
            <span className="font-semibold">ZCIS UNIVERSE SCAN</span>
          ...
          <div className="flex flex-wrap items-center gap-2 text-[12px] text-fg-secondary">
            {summary.scanned != null && (
              <span>{summary.scanned} ZCIS instruments scanned</span>
            )}
            ...
            {summary.asOfDate && (
              <>
                <span className="text-fg-faint" aria-hidden>·</span>
                <span>As of {summary.asOfDate}</span>
              </>
            )}
      ...
      <div className="grid grid-cols-1 gap-3 px-6 pt-4 lg:grid-cols-3">
        <SummaryCallout
          ...
          count={highCount}
          hint="Stems where the 252d-rolling ZCIS rate sits above its trailing-year mean by ≥ threshold."
        />
        <SummaryCallout
          tone="extreme-down"
          label="Biggest moves (low)"
          count={lowCount}
          hint="Stems where the 252d-rolling ZCIS rate sits below its trailing-year mean by ≥ threshold."
        />
        <SummaryCallout
          tone="elevated"
      ...
      <div className="px-6 pt-4">
        <RankedTable
          ...
          onOpenInBuild={(row) => {
            // Open the per-pillar inflation_swap_rate_level primitive for
            // the selected (curve_family, tenor).  The companion primitive
            // surfaces the per-stem deep-dive the scanner ranks against.
            const ctx = encodeURIComponent(
              JSON.stringify({
                tools: [
                  {
                    tool: 'calculate_inflation_swap_rate_level_tool',
                    params: {
      ...
      <LineageFooter
        lineage={{
          ...
          asOf: summary.asOfDate ?? undefined,
          freshness: 'fresh',
        }}
      />
    </div>
...
function IndexFamilyCard({
  rows,
}: {
  rows: ReadonlyArray<ScanInflationSwapsExtremesResultRow>;
}) {
  // Surface the index-family heterogeneity caveat at a glance — the desk
  // sees WHICH inflation indices showed up in the top-N so the load-bearing
  // CPI-U / HICPxT / RPI caveat is visible without scrolling.
  const families = new Set<string>();
  for (const r of rows) {
    const meta = zcisFamilyFor(r.curve_family);
    if (meta) families.add(meta.indexShort);
  }
  return (
    <div className="card flex h-full flex-col gap-2 px-4 py-3">
      <span className="kicker text-fg-muted">INDEX FAMILY CAVEAT</span>
...
function RankedTable({
  ...
  isLoading?: boolean;
  errorMessage?: string;
  onOpenInBuild: (row: ScanInflationSwapsExtremesResultRow) => void;
}) {
  return (
        ...
        <table className="w-full min-w-[760px] text-[12.5px]">
          <thead>
            <tr className="border-b border-line-subtle text-left text-[10.5px] uppercase tracking-wide text-fg-muted">
              ...
              <th className="px-3 py-2 text-right font-normal">Rate (mid)</th>
              <th className="px-3 py-2 text-right font-normal">1D Δ (bp)</th>
              <th className="px-3 py-2 text-right font-normal">1M Δ (bp)</th>
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-1.5 text-fg-primary">
                        ...
                        <span className="font-mono text-[12px]">{row.curve_family}</span>
                      </div>
                    </td>
                    <td className="px-3 py-2 font-mono text-fg-secondary">{row.tenor}</td>
                    <td className="px-3 py-2 text-fg-secondary">
                      {meta?.indexShort ?? row.underlying_index ?? '—'}
                    </td>
                    <td className="px-3 py-2 text-right font-mono tabular-nums text-fg-primary">
                      {formatRatePct(row.zcis_rate_pct)}
                    </td>
                    <td className={`px-3 py-2 text-right font-mono tabular-nums ${toneTextClass(toneForChangeBps(row.daily_change_zcis_rate_bps))}`}>
                      {formatChangeBps(row.daily_change_zcis_rate_bps)}
                    </td>
                    <td className={`px-3 py-2 text-right font-mono tabular-nums ${toneTextClass(toneForChangeBps(row.monthly_change_zcis_rate_bps))}`}>
                      {formatChangeBps(row.monthly_change_zcis_rate_bps)}
                    </td>
    ...
    <div className="card flex flex-col gap-3 px-4 py-4">
      ...
      <ul className="flex flex-col gap-2 text-[12px] text-fg-secondary">
        <li className="flex items-center gap-2">
          <span className={`h-2 w-3 rounded-sm ${toneTextClass('extreme-up')} bg-coral-400/80`} aria-hidden />
      ...
      <div className="flex items-start gap-2 rounded-md border border-line-subtle bg-surface-overlay px-3 py-2 text-[11px] leading-snug text-fg-secondary">
        ...
        <span>
          Click <span className="font-semibold text-fg-primary">Open in Build</span> on any row to drill into the per-pillar
...
function toneForChangeBps(
  bps: number | null | undefined,
): 'neutral' | 'positive' | 'negative' {
  if (bps == null || Number.isNaN(bps) || bps === 0) return 'neutral';
  // Inflation-swap RATE going UP = priced-in inflation going UP; the desk
  // colours an upside move as "negative" (red/coral on the duration side)
  // to mirror the sovereign-yield convention used across the catalogue.
