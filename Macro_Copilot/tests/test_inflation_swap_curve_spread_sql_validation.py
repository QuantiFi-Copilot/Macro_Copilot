#!/usr/bin/env python3
"""
test_inflation_swap_curve_spread_sql_validation.py — Same-curve
ZCIS curve-spread validator.

Validate ``calculate_inflation_swap_curve_spread`` against an
...
per-trade-date ZCIS curve-spread math without going through any of
the Python tool's helpers, so a mismatch surfaces a real
...
yielding 21 unique same-curve ``(short, long)`` pairs per curve
...
order on every run — every supported same-curve pair is
exercised against the SQL baseline rather than sampled.

Adversarial probes (5):
  1. Cross-curve attempt (USD_ZCIS short vs EUR_ZCIS long): the
     input layer rejects this because there is only a single
     ``curve_family`` field.  The schema validation must prevent
     any encoding of mismatched curve families.
...
# Full deterministic coverage of the ZCIS curve-spread universe.
# 3 curve families × C(7, 2) = 21 same-curve pairs each = 63 cases.
# Stable curve_family-then-(short,long) ordering so failures are
...
def _enumerate_same_curve_pairs(curve_family: str) -> List[Case]:
    ...
    for i, short_t in enumerate(sorted_pillars):
        for long_t in sorted_pillars[i + 1:]:
            pairs.append((curve_family, short_t, long_t))
    return pairs
...
# tolerance bands hold here.  ZCIS rates flow through one fewer
# layer of bond arithmetic than breakevens, so display deltas are
# typically tighter; keep the same tolerances as a safe upper
# bound.
TOLERANCE_BY_FIELD = {
    "spread_bps": 0.021,
    "change_1d_bps": 0.021,
...
def sql_baseline(
    ...
    *,
    ...
    """Independent SQL reproduction of the same-curve ZCIS tenor
    spread math.
    ...
    Algorithm (mirrors the Python tool's composition shape EXACTLY):
      ...
      3. Apply the per-trade-date difference:
            spread_bps = (long_pct - short_pct) * 100
    ...
    # computation in calculate_inflation_swap_curve_spread).  The
    ...
    # lookback + 378 (curve layer) + 378 (level layer) = +756
    ...
    baseline_sql = text(
        ...
        spread_rows AS (
            SELECT
                ...
                long_pct,
                ROUND(
                    ((long_pct - short_pct) * 100)::numeric,
                    2
...
def assert_inverted_tenor_rejected_by_schema() -> List[str]:
    ...
    failures.append(
        ...
        "short_tenor='10Y' AND long_tenor='2Y' (a curve inversion "
...
def assert_unknown_pillar_handling(
    ...
    *,
    ...
    failures: List[str] = []
    try:
        tool_result = calculate_inflation_swap_curve_spread(
            engine=engine,
...
def assert_four_conjunct_select_guard_inherited(
    ...
    NOT leak into either leg of the curve-spread compute.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Validate the inflation_swap_curve_spread tool against "
            "direct SQL."
        ),
    )
    ...
    print("=" * 80)
    print("ZCIS INFLATION_SWAP_CURVE_SPREAD TOOL — SQL VALIDATION")
    print("=" * 80)
    ...
    if args.cases == DEFAULT_CASE_COUNT and len(cases) != DEFAULT_CASE_COUNT:
        ...
        msg_lines = [
            "FATAL: Incomplete ZCIS curve-spread coverage — live pool "
