...
(ADR 0013) — one row per configured strip position on ONE
...
class FuturesStripSnapshotRow(BaseModel):
    ...
    row_methodology_card: str = Field(
        ...
        min_length=1,
        description=(
            "Per-row P5 / ADR 0013 disclosure. Names the underlying "
...
class FuturesStripSnapshotOutput(BaseModel):
    ...
    Wire shape:
      ...
      - ``methodology_disclosure``: the output-level P5 / ADR 0013
    ...
    short_rate_regime: str = Field(
        ...
        description=(
            ...
            "on the methodology disclosure string per P5 + ADR 0013."
        ),
    )
    ...
    methodology_disclosure: str = Field(
        ...
        min_length=1,
        description=(
            "Output-level P5 / ADR 0013 caveat carried on every "
            ...
            "quarterly; ADR 0013 V1 monitors-only). Required so "
