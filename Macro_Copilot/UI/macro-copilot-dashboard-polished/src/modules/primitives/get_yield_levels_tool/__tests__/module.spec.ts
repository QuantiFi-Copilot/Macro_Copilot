...
// ============================================================================
// src/modules/primitives/get_yield_levels_tool/__tests__/module.spec.ts
// ----------------------------------------------------------------------------
...
// Plus the dual-view contract checks (rendering_density.md §1) and the
// migration-mode backward-compat lock (MIGRATION_RULES §6) — the
// ``yield_level`` Monitor widget's id + paramFields must remain.
// ============================================================================

import { assertStandardModuleInvariants } from '../../../__test-utils';
import { MODULE } from '../module';

const FOLDER = 'get_yield_levels_tool';
...
check('claims custom_build_surface tier', () => {
  if (!MODULE.tiers.includes('custom_build_surface')) {
    throw new Error(
      `tiers missing 'custom_build_surface'; got tiers=${JSON.stringify(MODULE.tiers)}`,
    );
  }
});
...
// Dashboards persisted these.
// ----------------------------------------------------------------------------

check('yield_level Monitor widget is preserved verbatim', () => {
  const widgets = MODULE.monitorWidgets ?? [];
  const yieldLevel = widgets.find((w) => w.id === 'yield_level');
  if (!yieldLevel) {
    throw new Error(
      "monitorWidgets is missing the 'yield_level' widget.  Per MIGRATION_RULES §6 the widget id is the backward-compat lock and must not be renamed or removed.",
    );
  }
  if (!yieldLevel.parameterized) {
    throw new Error("'yield_level' widget MUST stay parameterized: true.");
  }
  const fieldNames = (yieldLevel.paramFields ?? []).map((f) => f.name).sort();
  const required = ['curve_family', 'tenor', 'lookback_days'].sort();
  for (const r of required) {
    if (!fieldNames.includes(r)) {
      throw new Error(
        `'yield_level' widget paramFields must include '${r}'; got [${fieldNames.join(', ')}].`,
      );
    }
  }
});
