...
// bond-futures extremes.  Mirrors the scan_inflation_swaps_extremes /
// scan_inflation_linkers_extremes sibling shape (identity row → top-right
// summary cards → controls strip → distribution histogram → ranked detail
// table → methodology card → lineage footer) but the bond-futures wire is
// MULTI-METRIC (price LEVEL / 1-day price CHANGE / volume LEVEL / open-
// interest LEVEL — each independently ranked).  Reuses the shared
// ControlsStrip / MethodologyCard / LineageFooter / FreshnessPill elements
// so the chrome matches the rest of the Build catalogue.
//
...
import type { ScanBondFuturesExtremesResultRow } from '@/types/rates';
...
const CURVE_FAMILY_OPTIONS: ReadonlyArray<{ value: string; label: string }> = [
  { value: ALL_CURVE_FAMILIES_VALUE, label: 'All bond futures (UST / DE / UK / JP / FR / IT / ES / CA / AU)' },
  ...BOND_FUTURES_SCANNER_CURVE_OPTIONS,
];
...
const BuildExtended: React.FC<BuildExtendedProps> = ({
  ...
  return (
    <div
      ...
      data-testid="bond-futures-scanner-extended"
    >
      {/* ---------- Title section ---------- */}
      <section className="grid grid-cols-1 gap-4 border-b border-line-subtle px-6 pt-5 pb-5 lg:grid-cols-[1fr_auto]">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-[11px] uppercase tracking-wide text-fg-muted">
            ...
            <span className="font-semibold">BOND FUTURES UNIVERSE SCAN</span>
          ...
          <div className="flex items-baseline gap-3">
            <h1 className="text-[24px] font-semibold leading-tight text-fg-primary">
              Front-Month Bond Futures · Extremes
            </h1>
          ...
          <div className="flex flex-wrap items-center gap-2 text-[12px] text-fg-secondary">
            {summary.scanned != null && (
              <span>{summary.scanned} stems scanned</span>
            )}
      ...
      <div className="px-6 pt-4">
        <RankedTable
          ...
          onOpenInBuild={(row) => {
            // Open the per-contract futures_price_level primitive for the
            // selected (curve_family, contract_code).  The companion
            // primitive surfaces the per-stem deep-dive the scanner ranks
            // against.
            const ctx = encodeURIComponent(
              JSON.stringify({
                tools: [
                  {
                    tool: 'futures_price_level_tool',
                    params: {
      ...
      <LineageFooter
        lineage={{
          toolName,
          version: 'v1',
          kind: 'Universe scanner (deterministic · multi-metric)',
...
function MetricBreakdownCard({
  ...
  // a glance whether today's extremity is concentrated in price (the
  // headline question) or in volume / OI (positioning signal).  Per the
  // catalog guardrail this is a four-metric scan; the V1 caveat lives on
  // the methodology card below.
  ...
  return (
    <div className="card flex h-full flex-col gap-1.5 px-4 py-3">
      <span className="kicker text-fg-muted">VS V1 LIMITATIONS (ADR 0013)</span>
      {entries.length === 0 ? (
        ...
        <ul className="flex flex-col gap-0.5 text-[11.5px] text-fg-secondary">
          {entries.map(([metric, count]) => (
            <li key={metric} className="flex items-baseline justify-between gap-2">
...
function RankedTable({
  ...
  isLoading?: boolean;
  errorMessage?: string;
  onOpenInBuild: (row: ScanBondFuturesExtremesResultRow) => void;
}) {
  return (
        ...
        <table className="w-full min-w-[920px] text-[12.5px]">
          <thead>
            <tr className="border-b border-line-subtle text-left text-[10.5px] uppercase tracking-wide text-fg-muted">
              ...
              <th className="px-3 py-2 text-left font-normal">Units</th>
          ...
          <tbody>
            {errorMessage ? (
              ...
              rows.map((row) => {
                ...
                const meta = bondFuturesFamilyFor(row.curve_family);
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-fg-secondary">
                      {meta?.countryLabel ?? row.curve_family}
                    </td>
  ...
  return (
    <div className="card flex flex-col gap-3 px-4 py-4">
      ...
      <div className="flex items-start gap-2 rounded-md border border-line-subtle bg-surface-overlay px-3 py-2 text-[11px] leading-snug text-fg-secondary">
        ...
        <span>
          Click <span className="font-semibold text-fg-primary">Open in Build</span> on any row to drill into the per-contract
          <code className="mx-1 rounded bg-bg-surface px-1 font-mono text-[10.5px]">
            futures_price_level_tool
          </code>
