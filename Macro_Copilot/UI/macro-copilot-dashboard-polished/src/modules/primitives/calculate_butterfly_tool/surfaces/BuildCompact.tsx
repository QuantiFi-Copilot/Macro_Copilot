...
// this is the same-curve sovereign butterfly's.  Mounted as a node body
...
const BuildCompact: React.FC<BuildCompactProps> = ({
  ...
  // The sovereign butterfly Output does NOT carry per-tenor strings (only
  // the ``butterfly_label``), so the per-tenor identity in the identity
  // row is reconstructed from the request params.
  ...
  //   primary:   "UST · 2-5-10 FLY"
  //   secondary: "US Treasuries curve curvature"
  const shortLabel = familyMeta?.shortLabel ?? curveFamily;
  const primary = familyMeta
    ? `${shortLabel} · ${triplet} FLY`
    : `${curveFamily || '—'} · ${triplet} FLY`;
  const secondary = familyMeta
    ? `${familyMeta.longLabel} curve curvature`
    : 'Sovereign curve curvature';

  return (
    <BuildCompactShell
      toolDisplayName="Butterfly (Sovereign)"
      statusPill="SNAPSHOT"
      ...
      caveatText={SOVEREIGN_BUTTERFLY_COMPACT_CAVEAT}
      asOf={cm?.as_of_date}
      freshness="fresh"
      onExpand={onExpand}
      size={size}
