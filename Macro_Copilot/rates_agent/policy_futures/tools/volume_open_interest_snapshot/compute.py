...
``policy_futures`` domain (ADR 0013 — strip-position-keyed monitors;
...
Every response carries a ``methodology_disclosure`` field with:
  ...
  - the P5 / ADR 0013 caveat verbatim — "this is the rolling-generic
    ...
    (ADR 0013 — policy_futures V1 ships strip-position-keyed monitors
    only)",
...
_METHODOLOGY_DISCLOSURE_TEMPLATE: str = (
    ...
    "mixes contracts across rolls (ADR 0013 — policy_futures V1 "
...
def calculate_volume_open_interest_snapshot(
    ...
    expiry_date, contract_size) and the P5 / ADR 0013 methodology
