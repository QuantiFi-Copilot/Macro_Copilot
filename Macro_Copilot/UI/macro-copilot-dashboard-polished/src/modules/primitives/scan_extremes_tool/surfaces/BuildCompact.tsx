// ============================================================================
// surfaces/BuildCompact.tsx — Compact Build view for ``scan_extremes_tool``.
...
// Sibling shape: ../../scan_inflation_swaps_extremes_tool/surfaces/BuildCompact.tsx.
//
...
// the top-5 ranked rows; anything beyond is reached via the expand
// affordance / "View all N" link.
...
const BuildCompact: React.FC<BuildCompactProps> = ({
  ...
  return (
    <article
      ...
      style={{ minHeight }}
      data-testid="sovereign-scanner-compact"
    >
      {/* ---------- Header ---------- */}
      <header className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[13px] font-semibold text-fg-primary">
              <span className="truncate">SOVEREIGN YIELD EXTREMES</span>
        ...
        {onExpand && (
          <button
            type="button"
            onClick={onExpand}
            aria-label="Open extended view of sovereign yield extremes"
      ...
      {errorMessage ? (
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-2">
          {/* Column headings */}
          <div className="grid grid-cols-[20px_1.4fr_0.9fr_0.5fr_auto_auto] items-center gap-2 px-1 text-[10.5px] uppercase tracking-wide text-fg-muted">
            <span>#</span>
            <span>Country</span>
            <span>Curve</span>
            <span>Tenor</span>
            <span className="text-right">Yield (Mid)</span>
              ...
              visibleRows.map((row) => {
                ...
                return (
                  <div
                    ...
                    className="grid grid-cols-[20px_1.4fr_0.9fr_0.5fr_auto_auto] items-center gap-2 border-t border-line-subtle px-1 py-1.5 text-[12.5px]"
                  >
                    <span
                    ...
                    <span className="flex min-w-0 items-center gap-1.5 text-fg-primary">
                      ...
                      <span className="truncate">{instrumentCountryLabel(row)}</span>
                    </span>
                    <span className="font-mono text-[12px] text-fg-secondary truncate">
      ...
      <footer className="flex items-center justify-between gap-3 text-[11px]">
        <div
          ...
          title={SCAN_EXTREMES_COMPACT_CAVEAT}
        >
          <Info size={12} strokeWidth={1.5} className="shrink-0 text-fg-muted" aria-hidden />
          <span className="truncate">{SCAN_EXTREMES_COMPACT_CAVEAT}</span>
        </div>
  ...
  return (
    <div className="flex min-h-[180px] flex-1 flex-col gap-2">
      {[0, 1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className="grid grid-cols-[20px_1.4fr_0.9fr_0.5fr_auto_auto] items-center gap-2 border-t border-line-subtle px-1 py-1.5"
        >
