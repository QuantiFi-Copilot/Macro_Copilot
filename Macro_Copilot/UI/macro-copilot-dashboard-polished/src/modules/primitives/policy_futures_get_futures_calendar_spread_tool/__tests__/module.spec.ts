...
// ============================================================================
// src/modules/primitives/policy_futures_get_futures_calendar_spread_tool/__tests__/module.spec.ts
// ----------------------------------------------------------------------------
...
// sibling policy_futures_get_futures_butterfly_simple_tool module test
// shape adapted for the calendar-spread's tier-set + Monitor widget id.
// ============================================================================

import { assertStandardModuleInvariants } from '../../../__test-utils';
import { MODULE } from '../module';

const FOLDER = 'policy_futures_get_futures_calendar_spread_tool';
...
check('claims monitor_surface tier', () => {
  if (!MODULE.tiers.includes('monitor_surface')) {
    throw new Error(
      `tiers missing 'monitor_surface'; catalog entry required_tiers includes monitor_surface per the desk-canonical STIR-slope eligibility.  Got tiers=${JSON.stringify(MODULE.tiers)}`,
    );
  }
});
...
check('monitorWidgets carries the Policy Futures Calendar Spread tile', () => {
  ...
  if (!ids.includes('policy_futures_calendar_spread')) {
    throw new Error(
      `monitorWidgets missing the 'policy_futures_calendar_spread' entry; got ids=${JSON.stringify(ids)}.`,
    );
  }
});
