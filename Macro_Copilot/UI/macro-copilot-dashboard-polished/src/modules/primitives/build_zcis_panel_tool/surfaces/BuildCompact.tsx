...
// the chip strip is a wrong-scope tripwire, not a roster.
...
const BuildCompact: React.FC<BuildCompactProps> = ({
  ...
  const { data, isLoading, errorMessage } = useZcisPanel({
    ...
    tenorsCsv: params.tenors || DEFAULT_TENORS_CSV,
  ...
  return (
    <article
      ...
      style={{ minHeight }}
      data-testid="zcis-panel-compact"
    >
      {/* ---------- Header ---------- */}
      <header className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[13px] font-semibold text-fg-primary">
              <span className="truncate">ZCIS PANEL</span>
        ...
        {onExpand && (
          <button
            type="button"
            onClick={onExpand}
            aria-label="Open extended view of the ZCIS panel contract"
