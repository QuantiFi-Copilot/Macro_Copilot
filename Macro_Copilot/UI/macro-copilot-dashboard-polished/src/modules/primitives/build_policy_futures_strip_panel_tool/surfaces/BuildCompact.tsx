...
// the chip strip is a wrong-scope tripwire, not a roster.  Chips carry
// the desk stem read ('SFR3'), with the raw flat key on the tooltip.
...
const BuildCompact: React.FC<BuildCompactProps> = ({
  ...
  // Blank family / position scope → full SOFR/SONIA/Euribor × 1..8
  // universe (the backend treats omitted scope as ALL).
  const fallbackWindow = defaultWindow();
  const { data, isLoading, errorMessage } = usePolicyFuturesStripPanel({
    curveFamiliesCsv: params.curve_families || DEFAULT_CURVE_FAMILIES_CSV,
    stripPositionsCsv: params.strip_positions || DEFAULT_STRIP_POSITIONS_CSV,
    ...
    calendarPolicy: params.calendar_policy || undefined,
  ...
  return (
    <article
      ...
      style={{ minHeight }}
      data-testid="policy-futures-strip-panel-compact"
    >
      {/* ---------- Header ---------- */}
      <header className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 flex-1 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-[13px] font-semibold text-fg-primary">
              <span className="truncate">POLICY-FUTURES STRIP PANEL</span>
        ...
        {onExpand && (
          <button
            type="button"
            onClick={onExpand}
            aria-label="Open extended view of the policy-futures strip panel contract"
      ...
      {errorMessage ? (
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3">
          ...
          {/* Column-chip strip — desk stem labels (raw key on tooltip) */}
      ...
      <footer className="flex items-center justify-between gap-3 text-[11px]">
        ...
        <div className="flex shrink-0 items-center gap-3 text-fg-muted">
          {data && <span>As of {data.end_date}</span>}
