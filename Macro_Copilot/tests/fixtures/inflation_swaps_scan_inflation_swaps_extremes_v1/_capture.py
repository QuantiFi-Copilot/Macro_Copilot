...
production output of ``calculate_scan_inflation_swaps_extremes``
...
float tolerance) so subsequent commits that touch the ZCIS
...
ZCIS universe data exposes idiosyncrasies (per-currency ZCIS
holiday patterns, days where one curve family reports
independently, NaN field values from the upstream feed, the
tenor-coverage symmetry across the 21-stem V1 ZCIS universe) that
...
3. Calls ``calculate_scan_inflation_swaps_extremes`` (against
...
``test_scan_inflation_swaps_extremes_parity.py`` is fully
...
from rates_agent.inflation_swaps.tools.scan_inflation_swaps_extremes.compute import (  # noqa: E402
...
# CASE DEFINITIONS
# ===========================================================================
# One representative scan: the full default ZCIS universe with
...
# column), and disclosure-attach paths.  Add more entries here to
# pin additional curve-family subsets.
#
# ``as_of_date`` is pinned to 2026-04-08 — the latest live DB
# anchor at the time of the round-1 build (matches the linker
# scanner's parity-fixture pinned date).  This makes the captured
# ranking DETERMINISTIC across capture runs (re-running the
# capture on a different wall-clock day still produces the same
# numbers given the same DB state).
...
def _reference_row_dict(
    ...
    return {
        ...
        "underlying_index": (
            str(underlying_index)
            if underlying_index is not None else None
        ),
    }
...
def _capture_one(case: dict, engine, captured_at: str, db_name: str) -> dict:
    ...
    params = ScanInflationSwapsExtremesInput(**case["params"])
    ...
    if raw_field_df.empty:
        raise RuntimeError(
            ...
            f"fetch_scan_universe returned 0 {field_name} rows. "
            "Check that the ZCIS universe is loaded in the DB."
        )
    ...
    if raw_ref_df.empty:
        raise RuntimeError(
            f"Capture failed for {case['fixture_name']}: "
            f"fetch_scan_universe_reference returned 0 rows. "
            "Check that the ZCIS universe's instrument_master + "
            "view rows exist."
        )
    ...
    expected_output = calculate_scan_inflation_swaps_extremes(
