...
class TestComposerPromptContent:
    ...
    # portion (raised to 46k) + ~10k of instructions → 56k bounds the
    # worst case, still leaving >140k for tool messages on a 200k
    # window.  Tracks the catalogue cap in lock-step (40k→49k→55k→56k as
    # the fleet grows; 2026-06-19 the +1k step is INSTRUCTION growth, not
    # catalogue: the bps/percent units idiom (convert_units→bps), the
    # event-study scalar-collapse + cross-instrument-align idioms — all
    # correctness guidance that closes disqualifying-class failures).
    ...
    _COMPOSER_PROMPT_TOKEN_CAP = 56_000
