...
- **Sparkline** — a small chart of the most-relevant time series (typically the same series the extended view's main chart shows, but condensed). **Semantic-contract guardrail (shape-dependent):** the sparkline exists ONLY when the wire actually carries a numeric series. Tools whose payload is a single classified observation, a categorical/SCD2 timeline, a per-meeting list, a ranked scan, or panel CONTRACT metadata must NOT fabricate a tape — they render a chartless KPI card, a table, or chips instead. Mechanically: `BuildCompactShell.chartPoints` is OMITTED for series-less wires (chartless layout) and passed an EMPTY ARRAY only when a series-shaped tool's window came back empty (the honest "No data in window" state). Established shape precedents: scanners → top-N ranked table; panel-builders → contract card (columns/rows/range KPIs + column-key chips); categorical timelines (OTR) → KPI row + windows table; snapshot classifiers (curve-move regime, WIRP meetings) → KPI-centric chartless cards.
