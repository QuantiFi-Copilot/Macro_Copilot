-- ============================================================================
-- Migration STUB: macro_data.tool_metadata for `calculate_otr_ofr_spread_tool`
...
SET
    theoretical_reference = $tref$STUB - see rates_agent/sovereign_bonds/tools/calculate_otr_ofr_spread/config.yaml methodology block.$tref$,
    ...
    updated_at = NOW()
WHERE tool_name = 'calculate_otr_ofr_spread_tool';
...
SELECT
    'calculate_otr_ofr_spread_tool', 'sovereign_bonds', 'snapshot', '{}'::jsonb,
    $tref$STUB$tref$,
    $klim$STUB$klim$,
    $narr$STUB$narr$
WHERE NOT EXISTS (SELECT 1 FROM macro_data.tool_metadata WHERE tool_name = 'calculate_otr_ofr_spread_tool');
