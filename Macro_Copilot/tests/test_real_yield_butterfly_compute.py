"""
test_real_yield_butterfly_compute.py — Unit tests for the linker
real_yield_butterfly primitive.

Mirrors ``test_real_yield_curve_spread_compute.py`` (the closest
sibling composition pattern in this domain) extended to the three-
leg butterfly construction.  Differs in the math under test:
per-trade-date FIXED simple-butterfly weighting
``belly - 0.5 * (short + long)``, reported in PERCENT (NOT bps) —
distinct from the BPS convention the sovereign butterfly uses,
because real yields are not multiplied by 100.
...
Covers:
  ...
  4. Convention overrides change behaviour (z-window, ddof, period
     offsets, yield rounding, default_field_name).
  ...
  6. Schema-layer behaviour: field_name defaults to None;
     ...
     validators reject all duplicate / inverted triplets.
  ...
  8. Per-trade-date alignment: dates where ANY endpoint has no
     data are dropped from the butterfly series.
  9. Butterfly formula correctness on synthetic input (hand-built
     arithmetic spot-check).
 10. Boundary-rounding parity:
     ``current_metrics.current_butterfly_pct`` equals
     ``time_series[-1].butterfly_pct`` AND
 ...
 13. Canonical TimeSeries outputs: PERCENT for the butterfly,
     Z_SCORE for the rolling z-score, with the documented
     series_name pattern.
 14. ffill semantics on a synthetic gap > ffill_limit_days.
 15. Z-score basics on a deterministic synthetic series.
...
# family identity guard before any inner level call fires.
...
def _patch_country_currency():
    with patch(
        "rates_agent.inflation_indexed_bonds.tools.real_yield_butterfly.compute._fetch_curve_family_country_currency",
...
def _build_legs_for_usd_tips_5s10s30s(
    *,
    short_real: float = 1.85,
    belly_real: float = 2.05,
    long_real: float = 2.25,
    short_drift: float = -0.30,
    belly_drift: float = -0.10,
    long_drift: float = +0.10,
) -> dict:
    """Default canonical three-leg fixture for USD_TIPS 5s10s30s
    real-yield butterfly.  Differentiated drifts produce a butterfly
    series that varies through time — necessary for the z-score /
    ...
    return {
        ...
        ("USD_TIPS", "10Y", "inflation_linker"): _synthetic_single_leg(
            days=days, base_pct=belly_real, drift_pct=belly_drift,
        ),
...
def _run(params, legs, *, config=None):
    with patch(
        ...
        return calculate_real_yield_butterfly(
...
class TestBundledConfig:
    ...
    def test_methodology_what_it_does_carries_disclosure(self):
        ...
        # Butterfly formula MUST be visible on the wire-disclosure.
        assert "belly_real_yield_pct" in text
        assert "short_real_yield_pct" in text
        assert "long_real_yield_pct" in text
        # Sign convention MUST be visible.
        assert "cheap" in text and "rich" in text
        # Fixed weighting MUST be visible.
        assert "0.5" in text
...
class TestComputeHappyPath:
    def test_default_config_returns_well_formed_output(self):
        legs = _build_legs_for_usd_tips_5s10s30s()
        ...
        assert cm["butterfly_label"] == "USD_TIPS 5s10s30s real-yield"
        assert cm["country"] == "US"
        assert cm["currency"] == "USD"

    def test_butterfly_formula_is_correct(self):
        """Hard-pin the butterfly formula on a synthetic fixture
        with constant endpoint real yields.  With short=1.50%,
        belly=2.00%, long=2.20%, the expected butterfly = 2.00 -
        0.5 * (1.50 + 2.20) = 2.00 - 1.85 = 0.15 pct.
        """
        days = 800
        legs = {
            ("USD_TIPS", "5Y", "inflation_linker"): _synthetic_single_leg(
        ...
        assert cm["belly_real_yield_pct"] == pytest.approx(2.00, abs=1e-4)
        assert cm["long_real_yield_pct"] == pytest.approx(2.20, abs=1e-4)
        # butterfly = 2.00 - 0.5 * (1.50 + 2.20) = 0.15
        assert cm["current_butterfly_pct"] == pytest.approx(0.15, abs=1e-4)
        # wings
        # wing_short = belly - short = 0.50, wing_long = long - belly = 0.20
        assert cm["wing_short_pct"] == pytest.approx(0.50, abs=1e-4)
        assert cm["wing_long_pct"] == pytest.approx(0.20, abs=1e-4)

    def test_butterfly_equivalent_to_half_doubled_form(self):
        """Sanity check: belly - 0.5*(short+long) ==
        0.5*(2*belly - short - long).  Both forms must agree at the
        bundled yield_round_decimals."""
        days = 800
        s, b, lo = 1.234, 2.345, 3.456
        legs = {
            ("USD_TIPS", "5Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=s, drift_pct=0.0,
            ),
            ("USD_TIPS", "10Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=b, drift_pct=0.0,
            ),
            ("USD_TIPS", "30Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=lo, drift_pct=0.0,
            ),
        }
        params = RealYieldButterflyInput(
            curve_family="USD_TIPS",
        ...
        out = _run(params, legs)
        cm = out["current_metrics"]
        expected_half = b - 0.5 * (s + lo)
        expected_doubled = 0.5 * (2 * b - s - lo)
        assert round(expected_half, 4) == round(expected_doubled, 4)
        assert cm["current_butterfly_pct"] == pytest.approx(
            round(expected_half, 4), abs=1e-4,
        )

    def test_negative_butterfly_handled(self):
        """When the belly underperforms (rich), the butterfly is
        negative.  The wire types must tolerate negatives."""
        days = 800
        legs = {
            ("USD_TIPS", "5Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=1.80, drift_pct=0.0,
            ),
            ("USD_TIPS", "10Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=1.60, drift_pct=0.0,
            ),
            ("USD_TIPS", "30Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=2.20, drift_pct=0.0,
            ),
        ...
        out = _run(params, legs)
        assert "error" not in out, out.get("error")
        # butterfly = 1.60 - 0.5*(1.80+2.20) = 1.60 - 2.0 = -0.40
        assert out["current_metrics"]["current_butterfly_pct"] < 0
...
class TestConventionOverrides:
    ...
    def test_period_offsets_override_changes_changes(self):
        # Pick drifts whose butterfly drift = belly - 0.5*(short+long)
        # is materially non-zero, otherwise a near-constant butterfly
        # series will produce identical period changes at any offset.
        # Default builder uses short=-0.30, belly=-0.10, long=+0.10 →
        # butterfly drift = -0.10 - 0.5*(-0.30 + 0.10) = 0.00, which
        # papers over the period-offset override.  Use a curvature-
        # changing pattern so daily_change_bps is genuinely sensitive
        # to the offset.
        legs = _build_legs_for_usd_tips_5s10s30s(
            short_drift=-0.40, belly_drift=+0.30, long_drift=-0.20,
        )
        params = RealYieldButterflyInput(
            curve_family="USD_TIPS",
            short_tenor="5Y",
    ...
    def test_yield_round_decimals_override_changes_precision(self):
        legs = _build_legs_for_usd_tips_5s10s30s(
            short_real=1.8765432,
            belly_real=2.0876543,
            long_real=2.3654321,
        )
        params = RealYieldButterflyInput(
            curve_family="USD_TIPS",
        ...
        v4 = out_4["current_metrics"]["current_butterfly_pct"]
        v6 = out_6["current_metrics"]["current_butterfly_pct"]
...
class TestInputSchemaBehaviour:
    ...
    def test_duplicate_short_belly_rejected(self):
    ...
    def test_duplicate_belly_long_rejected(self):
        with pytest.raises(Exception):
            RealYieldButterflyInput(
                curve_family="USD_TIPS",
                short_tenor="5Y",
    ...
    def test_duplicate_short_long_rejected(self):
        with pytest.raises(Exception):
            RealYieldButterflyInput(
                curve_family="USD_TIPS",
                short_tenor="10Y",
    ...
    def test_inverted_short_belly_rejected(self):
        """short > belly must be rejected."""
        ...
        assert "short_years < belly_years < long_years" in msg

    def test_inverted_belly_long_rejected(self):
        """belly > long must be rejected."""
        with pytest.raises(Exception) as exc_info:
            RealYieldButterflyInput(
                curve_family="USD_TIPS",
                short_tenor="5Y",
    ...
    def test_fully_reversed_triplet_rejected(self):
        """short=long_pos, belly=mid, long=short_pos must be rejected
        — no silent swap."""
        with pytest.raises(Exception) as exc_info:
            RealYieldButterflyInput(
                curve_family="USD_TIPS",
...
class TestFieldNameYamlFallthrough:
    """End-to-end proof that YAML default_field_name reaches ALL
    THREE inner level calls."""
...
class TestCompositionGuards:
    ...
    def test_identity_guard_runs_before_any_fetch(self):
        ...
        with patch(
            ...
            out = calculate_real_yield_butterfly(
                engine=None, params=params,
            )
        assert "error" in out
        assert fetch_calls == [], (
    ...
    def test_missing_belly_endpoint_returns_error_envelope(self):
        ...
        assert "current_metrics" not in out
        err = out["error"].lower()
        assert "belly-tenor" in err or "belly_tenor" in err
        assert "10y" in err

    def test_missing_long_endpoint_returns_error_envelope(self):
        legs = _build_legs_for_usd_tips_5s10s30s()
        legs[("USD_TIPS", "30Y", "inflation_linker")] = pd.DataFrame(
            columns=["trade_date", "field_value"],
        )
        params = RealYieldButterflyInput(
            curve_family="USD_TIPS",
    ...
    def test_identity_guard_surfaces_ambiguous_metadata(self):
        ...
        (country, currency) tuples for a curve_family, the helper
        would surface a controlled error rather than silently
        picking one."""
...
class TestPerTradeDateAlignment:
    ...
    def test_missing_endpoint_date_is_dropped(self):
        """Drop a multi-month batch of dates from the long-tenor leg
        far beyond the ffill_limit of the inner level primitive.
        Verify that an early portion of the butterfly series only
        contains dates present in ALL THREE endpoints."""
        legs = _build_legs_for_usd_tips_5s10s30s()

        # Drop ALL dates from the long-tenor leg before a cutoff so
        # those dates can't be aligned; the inner level primitive's
        # 5-day ffill cannot bridge a multi-month gap, so the
        # butterfly series for those dates is genuinely missing.
        long_df = legs[("USD_TIPS", "30Y", "inflation_linker")]
        cutoff_date = long_df["trade_date"].iloc[100]
        legs[("USD_TIPS", "30Y", "inflation_linker")] = long_df[
        ...
        for d in ts_dates:
            assert d >= first_long_date, (
                f"Butterfly series contains date {d!r} that "
                f"predates the long-tenor leg's first observation "
                f"({first_long_date!r}) — inner-join discipline "
                "broke."
            )
...
class TestBoundaryRoundingParity:
    def test_snapshot_pct_matches_bespoke_last_row_strictly(self):
        ...
        out = _run(params, legs)
        snap = out["current_metrics"]["current_butterfly_pct"]
        bespoke_last = out["time_series"][-1]["butterfly_pct"]
...
class TestMethodologyLabelThreading:
    def test_methodology_label_sourced_from_yaml(self):
        ...
        custom_text = (
            "CUSTOM-REAL-YIELD-BUTTERFLY-DISCLOSURE-77 belly - "
            "0.5*(short+long) real-yield curvature object"
        )
        out = _run(
            params, legs,
    ...
    def test_bundled_methodology_label_carries_formula_and_sign(self):
        legs = _build_legs_for_usd_tips_5s10s30s()
        params = RealYieldButterflyInput(
            curve_family="USD_TIPS",
            short_tenor="5Y",
        ...
        # Butterfly formula MUST be visible on the wire.
        assert "belly_real_yield_pct" in label
        assert "short_real_yield_pct" in label
        assert "long_real_yield_pct" in label
        # Sign convention MUST be visible.
        assert "cheap" in label and "rich" in label
...
class TestCanonicalTimeSeries:
    def test_canonical_butterfly_uses_percent_units_and_naming(self):
        legs = _build_legs_for_usd_tips_5s10s30s()
        params = RealYieldButterflyInput(
            curve_family="USD_TIPS",
            short_tenor="5Y",
        ...
        # PERCENT — distinct from the BPS convention the sovereign
        # butterfly uses.
        assert ts["units"] == "percent"
        assert (
            ts["series_name"]
            == "usd_tips_5y_10y_30y_real_yield_butterfly"
        )
    ...
    def test_canonical_butterfly_values_match_bespoke_pointwise(self):
        legs = _build_legs_for_usd_tips_5s10s30s()
        params = RealYieldButterflyInput(
            curve_family="USD_TIPS",
            short_tenor="5Y",
        ...
        out = _run(params, legs)
        canonical = out["time_series_butterfly"]["rows"]
        bespoke = out["time_series"]
        for i, (c_row, b_row) in enumerate(zip(canonical, bespoke)):
            assert c_row["date"] == b_row["date"], f"row {i}: date mismatch"
            assert c_row["value"] == b_row["butterfly_pct"], (
...
class TestFfillSemantics:
    ...
    row on the butterfly series (no synthetic carry-forward).
    """

    def test_long_gap_drops_butterfly_rows(self):
        legs = _build_legs_for_usd_tips_5s10s30s()

        # Carve out a ~20-business-day hole in the belly leg (well
        # beyond the 5-day ffill_limit) so the inner level primitive
        # cannot bridge it.
        belly_df = legs[("USD_TIPS", "10Y", "inflation_linker")]
        gap_start_idx = 300
        gap_end_idx = 320
        gap_dates = belly_df["trade_date"].iloc[gap_start_idx:gap_end_idx].tolist()
        legs[("USD_TIPS", "10Y", "inflation_linker")] = belly_df[
            ~belly_df["trade_date"].isin(gap_dates)
        ].reset_index(drop=True)
        ...
        # one must be absent from the bespoke butterfly series.
        for d in missing_dates_in_window[5:]:  # skip first 5 (ffill bridges)
            assert d.strftime("%Y-%m-%d") not in ts_dates, (
                f"Long-gap date {d!r} survived in the butterfly "
                "series — inner-join discipline must have broken."
            )
...
class TestZScoreBasics:
    ...
    def test_z_score_none_when_butterfly_constant(self):
        """A constant butterfly has zero variance → z-score must be
        ...
        out = _run(params, legs)
        # butterfly = 0.15 every day → z-score undefined (None).
        assert out["current_metrics"]["current_z_score"] is None


# ===========================================================================
# 15. Hand-built arithmetic spot-check on a deterministic series
# ===========================================================================

class TestHandBuiltArithmetic:
    """Construct a hand-built three-leg series and verify the
    butterfly's full time series matches ``belly - 0.5 * (short +
    long)`` at the latest row at full precision (10 decimals)."""

    def test_arithmetic_matches_belly_minus_half_wings(self):
        days = 400
        # Time-varying drifts so each row is distinct.
        legs = {
            ("USD_TIPS", "5Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=1.5000, drift_pct=-0.4000,
            ),
            ("USD_TIPS", "10Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=2.0000, drift_pct=-0.1000,
            ),
            ("USD_TIPS", "30Y", "inflation_linker"): _synthetic_single_leg(
                days=days, base_pct=2.5000, drift_pct=+0.2000,
            ),
        }
        params = RealYieldButterflyInput(
            curve_family="USD_TIPS",
        ...
        assert "error" not in out, out.get("error")
        cm = out["current_metrics"]
        s = cm["short_real_yield_pct"]
        b = cm["belly_real_yield_pct"]
        lo = cm["long_real_yield_pct"]
        expected = round(b - 0.5 * (s + lo), 4)
        # cm['current_butterfly_pct'] is also rounded to 4 decimals.
        assert cm["current_butterfly_pct"] == pytest.approx(
            expected, abs=1e-4,
        )
