...
primitive under the ``bond_futures`` domain (ADR 0011 — V1 monitors-
only; mirrors the ``futures_price_level`` shape).

Methodology disclosure
----------------------
Every response carries a ``methodology_disclosure`` field with:
  - the P5 / ADR 0011 caveat verbatim — "this is rolling-generic
...
_METHODOLOGY_DISCLOSURE_TEMPLATE: str = (
    ...
    "0011 — bond_futures V1 ships monitors only). The CTD "
...
def calculate_futures_volume_oi(
    ...
    expiry_date, security_name) and the P5 / ADR 0011 methodology
