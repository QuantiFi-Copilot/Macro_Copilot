"""
test_cross_market_inflation_swap_spread_parity.py — Snapshot
parity tests for calculate_cross_market_inflation_swap_spread
==================================================================

PR15 backfill — modelled on
``tests/test_inflation_swap_curve_spread_parity.py``; the only
shape difference is that the two inner level calls use DIFFERENT
ZCIS ``curve_family`` values at the SAME shared tenor.
...
``tests/fixtures/cross_market_inflation_swap_spread_v1/README.md``.
"""
...
FIXTURES_DIR = (
    ...
    / "cross_market_inflation_swap_spread_v1"
)
FLOAT_ABS_TOL = 1e-9
...
def test_cross_market_inflation_swap_spread_parity(fixture_path: Path) -> None:
    ...
    params = CrossMarketInflationSwapSpreadInput(**fx["input"]["params"])
    ...
    with patch(
        ...
        actual = calculate_cross_market_inflation_swap_spread(
