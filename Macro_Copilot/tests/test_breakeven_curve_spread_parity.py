"""
test_breakeven_curve_spread_parity.py — Snapshot parity tests for
                                        calculate_breakeven_curve_spread
========================================================================

PR15 backfill — modelled on
``tests/test_breakeven_inflation_simple_parity.py``; the only shape
difference is that the compose primitive issues 4 inner fetches and
4 inner country lookups per scenario (2 legs × 2 tenors).
...
``calculate_breakeven_inflation_simple`` and calls it.

Regenerating fixtures — see
``tests/fixtures/breakeven_curve_spread_v1/README.md``.
"""
...
FIXTURES_DIR = (
    Path(__file__).parent / "fixtures" / "breakeven_curve_spread_v1"
)
FLOAT_ABS_TOL = 1e-9
...
class _FrozenDateForBreakeven(date):
    ...
    ``breakeven_inflation_simple.compute.date`` so the inner spot
    primitive's one ``date.today()`` callsite becomes deterministic
    across ALL the inner endpoint calls this compose primitive makes
...
def test_breakeven_curve_spread_parity(fixture_path: Path) -> None:
    ...
    params = BreakevenCurveSpreadInput(**fx["input"]["params"])
    ...
    with patch(
        ...
        actual = calculate_breakeven_curve_spread(
