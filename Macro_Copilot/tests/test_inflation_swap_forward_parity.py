"""
test_inflation_swap_forward_parity.py — Snapshot parity tests for
                                        calculate_inflation_swap_forward
========================================================================

PR15 backfill — modelled on
``tests/test_inflation_swap_curve_spread_parity.py``; the only
difference is start/end tenor naming and the dual-compounding
geometric forward formula at the compose layer.  Seam topology is
identical.
...
``tests/fixtures/inflation_swap_forward_v1/README.md``.
"""
...
FIXTURES_DIR = (
    Path(__file__).parent / "fixtures" / "inflation_swap_forward_v1"
)
FLOAT_ABS_TOL = 1e-9
...
def test_inflation_swap_forward_parity(fixture_path: Path) -> None:
    ...
    params = InflationSwapForwardInput(**fx["input"]["params"])
    ...
    with patch(
        ...
        actual = calculate_inflation_swap_forward(
