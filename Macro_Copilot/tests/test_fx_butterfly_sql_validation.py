#!/usr/bin/env python3
"""
test_fx_butterfly_sql_validation.py
========================================

SQL-validates ``get_fx_butterfly`` against an independent SQL +
pandas baseline. Snapshot scope: current_butterfly_vol_pts, z_score,
observation_count. PR16 admission gate for the Phase E2 butterfly
smile primitive.

Mirror of test_fx_risk_reversal_sql_validation.py for the smile_point
∈ {'25B', '10B'} substrate.
"""
...
# fx_vol_smile coverage is USD-leg only — see risk_reversal sql validation for note.
...
def sql_baseline(
    ...
    return {
        "current_butterfly_vol_pts": current,
        "z_score": z,
...
def main() -> int:
    ...
    print("=" * 72)
    print(f"FX BUTTERFLY SQL VALIDATION — {len(DEFAULT_CASES)} case(s)")
    print("=" * 72)
    fail_count = 0
    skip_count = 0
    for pair, delta, tenor in DEFAULT_CASES:
        smile_point = f"{delta}B"
        try:
            tool_out = get_fx_butterfly(
                engine, FXButterflyInput(
        ...
        mismatches: List[str] = []
        a, b = tool["current_butterfly_vol_pts"], sql["current_butterfly_vol_pts"]
        if abs(a - b) > TOLERANCE_VOL_ABS:
            mismatches.append(
                f"current_butterfly_vol_pts: tool={a} sql={b} abs_diff={abs(a-b):.6f}"
            )
