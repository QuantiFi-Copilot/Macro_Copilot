...
The two shared compute libraries are split by the **P9 / OPR6 finance-blind boundary**, and the rule is single-sourced here (P10) so neither library's docstrings have to re-derive it:

| Library | Owns | May know about an asset class? | Primary consumer |
|---|---|---|---|
| [`shared/quant/`](../../shared/quant/) | **Finance-BLIND** statistical / model-fit numerics — OU/AR(1), PCA/SVD, regime HMM/GMM, descriptive GARCH, changepoint, Kalman DLM filter, Hurst, variance-ratio, the statistical tests. Pure numerics over numpy/pandas arrays. | **No** — naming an instrument/tenor/curve/asset-class concept or doing finance math here is a P9 breach. | The L3 **operators** (`shared/operators/`) wrap these behind typed-artifact signatures. |
| [`shared/analytics/`](../../shared/analytics/) | **Finance-AWARE** deterministic math the primitives call — spreads, yield levels, curve moves, panel assembly, day-count, the SE/CI/delta-method disclosure layers. | **Yes** — this is primitive-side, asset-class-aware territory. | The L2 **primitives** (`<agent>/<domain>/tools/`). |

Two rules keep the split honest:

1. **Math-once (P10 / plan §8.1).** A numeric the operator layer and a primitive both need is built **once in `shared/quant/`** and the analytics-side caller delegates to it — never a second copy. The committed examples: `shared.analytics.stats.ou_half_life` calls `shared.quant.ou.fit_ou_core`; `shared.analytics.stats.pca_yield_changes` delegates its SVD to `shared.quant.pca.pca_svd_core`. The analytics layer adds only its finance-aware layer on top (the SE/CI/delta-method gate, the tenor relabel + curve-position sign anchor). The `sovereign_bond_hmm`-style duplicated model fit is the banned anti-pattern.
2. **One-way imports.** `shared/quant/` must **never** import `shared/analytics/` (or any `rates_agent/` domain). The dependency arrow points analytics → quant only.

**Known residence exception — `rolling_ols`.** The finance-blind rolling-OLS numeric currently lives at [`shared/analytics/regression.py`](../../shared/analytics/regression.py)`::rolling_ols`, not in `shared/quant/`, even though it is finance-blind by the swap-test. It is **correctly single-sourced** (the `rolling_regression` operator and the `rolling_regression` / `beta_adjusted_spread` primitives all call the one function), so math-once holds; only its *residence* sits on the analytics side of the line. Relocating it to `shared/quant/regression.py` is a non-behavioural import-churn move deferred to a dedicated change; the math-once + one-way-import invariants above are what actually matter and both hold today. `shared/quant/__init__.py` links back to this section as the authoritative boundary statement.
...
|---|---|---|---|
| v1.2 | 2026-06-16 | Added the **quant vs analytics** boundary section (m26): single-sources the P9/OPR6 finance-blind split between `shared/quant/` (finance-blind numerics, operator-consumed) and `shared/analytics/` (finance-aware math, primitive-consumed); states the math-once + one-way-import rules with the committed `fit_ou_core` / `pca_svd_core` delegation examples; documents the `rolling_ols` residence exception (finance-blind but living in analytics, correctly single-sourced; relocation deferred). `shared/quant/__init__.py` links here as the authoritative boundary. | — |
