...
|---|---:|---|---|---|---|---|
| `event_study` | ~10 | active | routed-generic via `WorkflowResultCard` | rendered via `DagView` | rendered via `ResultsView → EventStudyDashboard` (specialised) | over-specified node chain (explicit `align` + `select_from_series_set`); user-reported "garbage outputs" likely backend-side, not frontend |
...
**Future enhancement (not currently in scope):** if the workflow's `ResultsView` adds a "show intermediate steps" toggle, non-terminal operator artifacts will surface via the same artifact-type widget registry. No per-operator code change required.
...
2. **`contextDecoder` typed-kind coverage** — the decoder probably classifies the 47 non-typed-renderer tools as `generic_builder` kind, which the canvas does handle. But any tool the decoder fails on falls to `DecodeError`. Need a `contextDecoder` audit + missing-kind additions. (Decoder lives at `src/components/build/primitive/contextDecoder.ts`.)
3. **Workspace fork → re-render with new params loop** — infrastructure exists (`WorkspaceOverridesProvider`) but end-to-end loop not yet verified.
4. **`event_study` / `backtest` backend payload quality** — user reported "garbage numbers". Frontend renders honestly; the fix is on the backend side.
5. **Operator intermediate-output surfacing** — currently the only way to see an intermediate operator output is to make it the terminal artifact. The "show intermediate steps" toggle is a future enhancement.
