"""FX butterfly snapshot compute path.

Mirror of fx_agent/vol/tools/risk_reversal/compute.py for the wing
leg of the smile. Key differences:

  - Resolves smile_point from delta_anchor (25 → '25B', 10 → '10B')
  - Output field is current_butterfly_vol_pts (positive = wings rich)
...
from fx_agent.vol.tools.butterfly.schemas import (
    FXButterflyInput,
    FXButterflyMetrics,
    FXButterflyOutput,
)
...
def get_fx_butterfly(
    ...
    smile_point = f"{delta}B"  # delta_anchor=25 → "25B" ; delta_anchor=10 → "10B"
    tenor = params.tenor
    ...
    if df.empty:
        raise ValueError(
            f"No FX butterfly data found for pair={pair}, "
    ...
    if df.empty:
        raise ValueError(
            f"FX butterfly data for {pair}/{smile_point}/{tenor} all-NaN "
            "after cleaning."
        )
    ...
    metrics = FXButterflyMetrics(
        ...
        current_butterfly_vol_pts=round(current, vol_decimals),
    ...
    return FXButterflyOutput(current_metrics=metrics).model_dump()
