// ============================================================================
// BreakevenButterflyWidget — Monitor bento tile for one bond-implied
// breakeven butterfly (3-point curvature on a single same-country pair).
// ----------------------------------------------------------------------------
// Parameterised on (pair, triplet, lookback_days) where ``pair`` is the
// linker curve_family that uniquely determines the same-country nominal
// counterparty (a breakeven butterfly is a same-country object) and
// ``triplet`` is the registered (short, belly, long) tuple selected by
// label (e.g. "5s10s30s").  Fetches the SAME typed-detail endpoint
// (/api/v1/rates/detail/breakeven-butterfly) the Build views use — the
...
// The inflation-compensation-curvature caveat is surfaced via the kicker.
// Mirrors the BreakevenInflationWidget shape.
// ============================================================================
...
export function BreakevenButterflyWidget({ params }: Props) {
  const fetchParams = useMemo<BreakevenButterflyDetailParams | null>(() => {
    ...
    // per-pair registry so the widget can't dispatch an invalid ordering.
    const tripletKey = typeof params.triplet === 'string' ? params.triplet : null;
    const triplets = BREAKEVEN_BUTTERFLY_TRIPLETS_BY_PAIR[linker] ?? [];
    const selected =
  ...
  const { data, error, isLoading } = useFetchDetailBreakevenButterfly(fetchParams);
  ...
  const triplet = tripletLabel(m.short_tenor, m.belly_tenor, m.long_tenor);
  ...
  return (
    <>
      <WidgetHeader
        kicker={`${pairLabel} · ${triplet.toUpperCase()} ${flag}`.trim()}
        title="Breakeven Butterfly"
        meta={
          <span
            className={cn(
          ...
          <div className="mt-1 flex items-center gap-2 font-mono text-[9.5px] text-fg-faint">
            ...
            <div className="relative h-1.5 flex-1 rounded-full bg-white/[0.04] ring-1 ring-line-subtle">
              {(() => {
                ...
                const pct = ((flyBps - m.low_252d_bps) / range) * 100;
                return (
                  <span
                    aria-hidden
...
function useFetchDetailBreakevenButterfly(
  params: BreakevenButterflyDetailParams | null,
) {
