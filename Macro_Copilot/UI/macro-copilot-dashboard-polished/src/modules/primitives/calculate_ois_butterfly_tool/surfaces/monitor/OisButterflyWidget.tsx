// ============================================================================
// OisButterflyWidget — Monitor bento tile for one same-curve OIS butterfly
// (3-point curvature on a single OIS par-swap curve family).
// ----------------------------------------------------------------------------
// Parameterised on (curve, triplet, lookback_days) where ``curve`` is the OIS
// curve_family (USD_SOFR_OIS / EUR_ESTR_OIS / GBP_SONIA_OIS / JPY_OIS /
// AUD_OIS / CAD_OIS) and ``triplet`` is the registered (short, belly, long)
// tuple selected by label (e.g. "2s5s10s").  Fetches the SAME typed-detail
// endpoint (/api/v1/rates/detail/ois-butterfly) the Build views use — the
...
// methodology card on the Extended view.  Mirrors the
// InflationSwapButterflyWidget shape, adapted for the OIS Output shape (no
// ``cm.short_tenor`` / ``cm.belly_tenor`` / ``cm.long_tenor`` strings — the
// per-tenor identity is reconstructed from the registry's selected triplet).
// ============================================================================
...
export function OisButterflyWidget({ params }: Props) {
  const fetchParams = useMemo<OisButterflyDetailParams | null>(() => {
    ...
    // "2s5s10s").  Look up the (short, belly, long) tuple from the per-curve
    // registry so the widget can't dispatch an invalid ordering.
  ...
  const { data, error, isLoading } = useFetchDetailOisButterfly(fetchParams);
  ...
  const triplet = tripletHyphenLabel(
    fetchParams.short_tenor,
    fetchParams.belly_tenor,
...
function useFetchDetailOisButterfly(
  params: OisButterflyDetailParams | null,
) {
