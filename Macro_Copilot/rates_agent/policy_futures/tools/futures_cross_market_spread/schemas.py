...
(ADR 0011) — matched-strip implied-rate differential between TWO
...
--------------------------------------------------------------------
Per the catalog (build_order 18) and ADR 0011, policy futures are
...
class FuturesCrossMarketSpreadInput(BaseModel):
    ...
    ADR 0011. The sign convention is "A minus B"; the validator
...
class FuturesCrossMarketSpreadCurrentMetrics(BaseModel):
    ...
    short_rate_regime_a: str = Field(
        ...
        description=(
            ...
            "disclosure string per P5 + ADR 0011 + the catalog's "
