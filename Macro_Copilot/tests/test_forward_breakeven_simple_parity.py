"""
test_forward_breakeven_simple_parity.py — Snapshot parity tests for
                                         calculate_forward_breakeven_simple
============================================================================

PR15 backfill — modelled on
``tests/test_breakeven_curve_spread_parity.py``; the only shape
difference is start/end tenor naming and the year-weighted forward
formula at the compose layer.  Seam topology is identical.
...
``tests/fixtures/forward_breakeven_simple_v1/README.md``.
"""
...
FIXTURES_DIR = (
    Path(__file__).parent / "fixtures" / "forward_breakeven_simple_v1"
)
FLOAT_ABS_TOL = 1e-9
...
def test_forward_breakeven_simple_parity(fixture_path: Path) -> None:
    ...
    params = ForwardBreakevenSimpleInput(**fx["input"]["params"])
    ...
    with patch(
        ...
        actual = calculate_forward_breakeven_simple(
