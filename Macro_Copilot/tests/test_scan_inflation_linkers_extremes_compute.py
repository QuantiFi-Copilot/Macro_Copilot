"""
test_scan_inflation_linkers_extremes_compute.py — Unit tests for the
                                                   linker universe-
                                                   wide extremes scan

Layer A offline deterministic tests for
``calculate_scan_inflation_linkers_extremes``.  Covers:
  ...
  6. Methodology disclosure: present on EVERY row AND on the
     ...
     INDEX-FAMILY + MARKET-STRUCTURE caveats; contains the morning-
     screen / real-yield-scope label.
  7. Per-row reference columns (maturity_date / country /
...
# Default V1 linker universe — matches the YAML's
# ``inflation_linker_curve_families`` whitelist on a per-tenor basis
...
# Reference rows the new fetcher returns for those stems.  Mirrors
# the live DB shape — country / maturity_date / vendor_ticker
# attached per (curve_family, tenor).
_DEFAULT_REFERENCE = [
    {
        "curve_family": "USD_TIPS", "tenor": "5Y",
        "contract_code": None, "maturity_date": date(2030, 10, 15),
        "country": "US", "vendor_ticker": "GTII5 Govt",
    },
    {
        "curve_family": "USD_TIPS", "tenor": "10Y",
        "contract_code": None, "maturity_date": date(2036, 1, 15),
        "country": "US", "vendor_ticker": "GTII10 Govt",
    },
    {
        "curve_family": "USD_TIPS", "tenor": "30Y",
        "contract_code": None, "maturity_date": date(2056, 2, 15),
        "country": "US", "vendor_ticker": "GTII30 Govt",
    },
    {
        "curve_family": "GBP_LINKER", "tenor": "5Y",
        "contract_code": None, "maturity_date": date(2031, 8, 10),
        "country": "UK", "vendor_ticker": "GTGBPII5Y Govt",
    },
    {
        "curve_family": "GBP_LINKER", "tenor": "10Y",
        "contract_code": None, "maturity_date": date(2035, 9, 22),
        "country": "UK", "vendor_ticker": "GTGBPII10Y Govt",
    },
    {
        "curve_family": "EUR_FR_LINKER", "tenor": "10Y",
        "contract_code": None, "maturity_date": date(2036, 3, 1),
        "country": "France", "vendor_ticker": "GTFRFII10Y Govt",
    },
    {
        "curve_family": "CAD_RRB", "tenor": "10Y",
        "contract_code": None, "maturity_date": date(2036, 12, 1),
        "country": "Canada", "vendor_ticker": "GTCADII10Y Govt",
    },
]
...
def _default_field_df(
    ...
    drifts = {
        # 30-year UK exhibits the largest upward drift → most extreme z.
        ("GBP_LINKER", "10Y"): 2.5,
        # USD_TIPS 5Y exhibits a moderate downward drift → 2nd in |z|.
        ("USD_TIPS", "5Y"): -1.8,
        # USD_TIPS 30Y exhibits a smaller positive drift.
        ("USD_TIPS", "30Y"): 1.0,
        # GBP_LINKER 5Y has a small negative drift.
        ("GBP_LINKER", "5Y"): -0.5,
        # The rest stay roughly flat.
        ("USD_TIPS", "10Y"): 0.1,
        ("EUR_FR_LINKER", "10Y"): 0.2,
        ("CAD_RRB", "10Y"): -0.1,
    }
    return _build_universe_df(
        stems=stems, per_stem_drift=drifts, base=1.5,
...
def _run(
    ...
    *,
    ...
    target = (
        ...
        "scan_inflation_linkers_extremes.compute"
    )
    with patch(
...
def _custom_config(**overrides) -> ToolConfig:
    ...
    return ToolConfig(
        tool=ToolMeta(
            name="scan_inflation_linkers_extremes_tool",
...
class TestBundledConfig:
    ...
    def test_convention_defaults(self):
        ...
        wl = cfg.convention_value("inflation_linker_curve_families")
        assert isinstance(wl, str)
        for cf in ["USD_TIPS", "GBP_LINKER", "EUR_FR_LINKER", "CAD_RRB"]:
            assert cf in wl
        # Nominal sovereign curve_families must NOT be in the whitelist.
        for cf in ["UST", "DE_BUND", "UK_GILT"]:
            assert cf not in wl
    ...
    def test_methodology_what_it_does_mentions_real_yield(self):
        cfg = load_tool_config(CONFIG_PATH)
        text = cfg.methodology.what_it_does.lower()
        assert "real-yield" in text or "real yield" in text
...
class TestComputeHappyPath:
    def test_default_config_returns_well_formed_output(self):
        ...
        ScanInflationLinkersExtremesOutput.model_validate(result)
    ...
    def test_results_ordered_by_abs_z_desc(self):
        ...
        result, _calls = _run(params)
        zs = [abs(r["z_score_real_yield"]) for r in result["results"]]
    ...
    def test_signal_matches_z_sign(self):
        ...
        for r in result["results"]:
            z = r["z_score_real_yield"]
            if z is not None and z > 0:
                assert r["signal"] == "EXTREME_HIGH"
            elif z is not None and z < 0:
                assert r["signal"] == "EXTREME_LOW"

    def test_fetcher_invoked_with_inflation_linker_instrument_type(self):
        """Load-bearing no-proxy guard: the fetcher MUST be invoked with
        instrument_type='inflation_linker'."""
        params = ScanInflationLinkersExtremesInput(top_n=5, min_abs_z_score=0.0)
        _result, calls = _run(params)
        assert calls["scan_universe"]["instrument_type"] == "inflation_linker"
        assert calls["scan_universe"]["field_name"] == "YLD_YTM_MID"
        assert calls["scan_reference"]["instrument_type"] == "inflation_linker"
...
class TestMethodologyDisclosure:
    ...
    def test_disclosure_on_every_row(self):
        params = ScanInflationLinkersExtremesInput(top_n=10, min_abs_z_score=0.0)
        result, _calls = _run(params)
        assert len(result["results"]) > 0
    ...
    def test_disclosure_contains_252_lookback(self):
        params = ScanInflationLinkersExtremesInput(top_n=5, min_abs_z_score=0.0)
        result, _calls = _run(params)
        # Catalog methodology guardrail: the lookback window must be
        # disclosed verbatim.
        assert "252" in result["methodology_disclosure"]
        for row in result["results"]:
            assert "252" in row["methodology_disclosure"]

    def test_disclosure_calls_out_real_yield_scope(self):
        params = ScanInflationLinkersExtremesInput(top_n=5, min_abs_z_score=0.0)
        result, _calls = _run(params)
        md = result["methodology_disclosure"].lower()
        # REAL-YIELD label — not breakeven, not nominal.
        assert "real-yield" in md or "real yield" in md
        # Explicit "NOT a breakeven scan" disclosure.
        assert "not a breakeven" in md
        # NOT a nominal-yield scan disclosure.
        assert "nominal-yield scan" in md or "nominal yield scan" in md
    ...
    def test_disclosure_index_family_caveat(self):
        ...
        # Specific index references for cross-country honesty.
        for ix in ["cpi-u", "hicp", "rpi"]:
            assert ix in md, f"index reference '{ix}' missing from disclosure"
    ...
    def test_disclosure_morning_screen_scope(self):
        ...
        assert "not a tactical" in md or "morning screen, not a tactical" in md
...
class TestReferenceColumns:
    def test_maturity_country_vendor_ticker_present(self):
        params = ScanInflationLinkersExtremesInput(top_n=10, min_abs_z_score=0.0)
        result, _calls = _run(params)
        for row in result["results"]:
    ...
    def test_reference_columns_match_lookup(self):
        ...
        for row in result["results"]:
            ...
            assert row["country"] == expected[key]["country"]
            assert row["vendor_ticker"] == expected[key]["vendor_ticker"]

    def test_missing_reference_row_leaves_none(self):
        """If the reference fetcher is missing a (curve_family, tenor),
        the corresponding output row has None reference columns —
        defensive; not an error."""
        params = ScanInflationLinkersExtremesInput(top_n=10, min_abs_z_score=0.0)
        # Drop GBP_LINKER 10Y from the reference rows.
        ref = pd.DataFrame([
            r for r in _DEFAULT_REFERENCE
            if (r["curve_family"], r["tenor"]) != ("GBP_LINKER", "10Y")
        ])
        result, _calls = _run(params, reference_df=ref)
        for row in result["results"]:
            if row["curve_family"] == "GBP_LINKER" and row["tenor"] == "10Y":
...
class TestFilteringAndRanking:
    def test_top_n_caps_output(self):
        params = ScanInflationLinkersExtremesInput(top_n=2, min_abs_z_score=0.0)
        result, _calls = _run(params)
    ...
    def test_min_abs_z_score_filters_below_threshold(self):
        ...
        result, _calls = _run(params)
        for row in result["results"]:
            assert abs(row["z_score_real_yield"]) >= 1.0
    ...
    def test_threshold_too_high_returns_error_envelope(self):
        ...
        assert "|z|" in result["error"] or "min_abs_z_score" in result["error"]

    def test_none_top_n_falls_back_to_yaml_default(self):
        params = ScanInflationLinkersExtremesInput(
            top_n=None, min_abs_z_score=0.0,
        )
        result, _calls = _run(params, config=_custom_config(default_top_n=3))
    ...
    def test_none_min_abs_z_score_falls_back_to_yaml_default(self):
        ...
        result, _calls = _run(
            params, config=_custom_config(default_min_abs_z_score=0.0),
        )
...
class TestDeterministicOrdering:
    def test_tie_break_curve_family_then_tenor_asc(self):
        """Two stems with identical |z| must order by curve_family asc
        then tenor asc (compute's tie-break)."""
        # Build a deliberately tied synthetic universe — two stems with
        # the same drift produce the same |z| at the latest aligned
        # row.
        stems = [
            ("USD_TIPS", "10Y"),
            ("GBP_LINKER", "10Y"),
        ...
        df = _build_universe_df(
            stems=stems, per_stem_drift={
                ("USD_TIPS", "10Y"): drift,
                ("GBP_LINKER", "10Y"): drift,
            },
            base=1.5, days=400, frozen_today=_FROZEN_TODAY,
        )
        ref = pd.DataFrame([
            r for r in _DEFAULT_REFERENCE
        ...
        # GBP_LINKER < USD_TIPS lexicographically — tie-break wins for
        # GBP_LINKER.
        assert cfs[0] == "GBP_LINKER", (
...
class TestConventionOverrides:
    def test_smaller_min_periods_admits_shorter_history(self):
        # Short history (only 50 trading days); default min_periods=60
        # would skip every stem.
        params = ScanInflationLinkersExtremesInput(top_n=5, min_abs_z_score=0.0)
...
class TestSchemaBehaviour:
    def test_nominal_sovereign_curve_family_refused(self):
        with pytest.raises(Exception) as exc_info:
            ScanInflationLinkersExtremesInput(
                curve_families=["USD_TIPS", "UST"],  # 'UST' is nominal
            )
        assert "whitelist" in str(exc_info.value) or "UST" in str(exc_info.value)

    def test_empty_curve_families_list_refused(self):
        with pytest.raises(Exception):
            ScanInflationLinkersExtremesInput(curve_families=[])
    ...
    def test_no_lookback_days_param(self):
        with pytest.raises(Exception):
            ScanInflationLinkersExtremesInput(lookback_days=180)

    def test_no_metrics_param(self):
        with pytest.raises(Exception):
            ScanInflationLinkersExtremesInput(metrics=["level"])

    def test_top_n_upper_bound(self):
        with pytest.raises(Exception):
            ScanInflationLinkersExtremesInput(top_n=999)

    def test_min_abs_z_score_negative_refused(self):
        with pytest.raises(Exception):
            ScanInflationLinkersExtremesInput(min_abs_z_score=-1.0)

    def test_frozen_model(self):
        """ConfigDict(frozen=True) — attribute writes must fail."""
        p = ScanInflationLinkersExtremesInput()
        with pytest.raises(Exception):
...
class TestEdgeCases:
    def test_empty_universe_returns_error(self):
        ...
        assert "error" in result
        assert "No linker real-yield data" in result["error"]

    def test_all_stems_below_min_periods_returns_error(self):
        # Only 10 days per stem — far below the default min_periods=60.
        short_df = _default_field_df(days=10)
        params = ScanInflationLinkersExtremesInput(top_n=5, min_abs_z_score=0.0)
        result, _calls = _run(params, field_df=short_df)
        assert "error" in result
        assert "scoreable" in result["error"] or "z-score" in result["error"]

    def test_future_as_of_date_returns_controlled_error(self):
        # as_of_date BEYOND the synthetic universe's last trading day.
        params = ScanInflationLinkersExtremesInput(
            top_n=5, min_abs_z_score=0.0,
