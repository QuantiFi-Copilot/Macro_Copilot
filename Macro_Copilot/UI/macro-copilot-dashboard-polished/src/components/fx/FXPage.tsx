    ...
    <div className="h-full min-h-0 overflow-y-auto pb-10">
      <div className="mx-auto w-full max-w-[1680px] px-6 py-6 lg:px-8 lg:py-7">
        <div className="mb-6 flex items-end justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-[12.5px] text-fg-secondary">
              Spot, momentum, z-score and forward-implied carry analytics from TimescaleDB.
            </p>
          </div>
        ...
        <div className="grid grid-cols-12 gap-4 lg:gap-5">
          ...
          <div className="card col-span-12 px-5 py-5">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="kicker mb-2">Carry monitor</p>
                <h2 className="text-[15px] font-semibold text-fg-primary">
                  1M forward-implied carry
                </h2>
              </div>
              <span className="text-[11px] text-fg-muted">
                tenor {data.carry.tenor}
              </span>
            </div>
            ...
            <div className="overflow-hidden rounded-xl border border-line-soft">
              <table className="w-full text-left text-[12px]">
                ...
                <tbody>
                  {data.carry.rows.map((row) => (
                    <tr key={row.pair} className="border-t border-line-subtle">
                      <Td strong>{row.pair}</Td>
                      <Td>{fmtNum(row.spot, row.pair.includes('JPY') ? 2 : 4)}</Td>
                      <Td>{fmtNum(row.forward_points, 2)}</Td>
                      <Td>{fmtNum(row.outright_forward, row.pair.includes('JPY') ? 2 : 4)}</Td>
                      <Td>{fmtNum(row.carry_bps_spot, 2)}</Td>
                      <Td>{fmtPct(row.carry_annualized_pct)}</Td>
                      <Td>{row.carry_signal}</Td>
                      <Td muted>{row.forward_date}</Td>
                    </tr>
                  ))}
                </tbody>
              </table>
